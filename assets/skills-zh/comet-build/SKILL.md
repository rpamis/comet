...
通过 subagent 创建实施计划，避免 planning skill 占用主 session 上下文。计划文件和执行反馈必须使用 `node "<comet-state-script>" get <name> language` 读取到的 Comet 配置产物语言。

**Subagent 指令**：

你是实施计划专家。基于以下输入创建实施计划：

1. **立即执行：** 使用 Skill 工具加载 Superpowers `writing-plans` 技能。禁止跳过此步骤。技能加载后，ARGUMENTS 必须包含：`Language: 使用 node "<comet-state-script>" get <name> language 读取到的 Comet 配置产物语言输出`
...
- 若用户选择 `executing-plans`：运行 `node "<comet-state-script>" set <name> subagent_dispatch null`，再运行 `node "<comet-state-script>" set <name> build_mode executing-plans`
- 若用户选择 `subagent-driven-development`：先确认当前平台存在可调用的真实后台 subagent / Task / multi-agent 调度能力；确认后先运行 `node "<comet-state-script>" set <name> subagent_dispatch confirmed`，再运行 `node "<comet-state-script>" set <name> build_mode subagent-driven-development`
...
运行 `node "<comet-state-script>" set <name> tdd_mode <tdd|direct>`
...
运行 `node "<comet-state-script>" set <name> review_mode <off|standard|thorough>`

`isolation` 是脚本级硬约束。full workflow 初始化时可以为 `null`，但只允许存在到本步骤之前。若保持 `null`，`build → verify` 的 guard 和 `node "<comet-state-script>" transition build-complete` 都会失败。full workflow 允许 `current`、`branch` 或 `worktree`，但 `current` 必须通过用户在 Step 2 显式选择后写入，不得静默默认。

`subagent_dispatch` 是脚本级硬约束。`build_mode: subagent-driven-development` 离开 build 阶段前必须同时满足 `subagent_dispatch: confirmed`，否则 `node "<comet-guard-script>" build --apply` 和 `node "<comet-state-script>" transition build-complete` 都会失败。

`tdd_mode` 是脚本级硬约束。full workflow 离开 build 阶段前 `tdd_mode` 必须已选择为 `tdd` 或 `direct`，否则 `node "<comet-guard-script>" build --apply` 和 `node "<comet-state-script>" transition build-complete` 都会失败。

`review_mode` 是脚本级硬约束。新建 full workflow 离开 build 阶段前 `review_mode` 必须已选择为 `off`、`standard` 或 `thorough`，否则 `node "<comet-guard-script>" build --apply` 和 `node "<comet-state-script>" transition build-complete` 都会失败。旧状态文件若没有该字段，按兼容路径继续，但恢复时应补写该字段。
...
- **current**：不创建新分支或 worktree，直接在当前 Git 分支执行。立即运行 `node "<comet-state-script>" set <name> isolation current`；该命令会把当前分支写入 `bound_branch`。如果当前是 detached HEAD，必须停止并让用户先切回真实分支，因为没有可审计的绑定分支。
...
- **branch**：使用 Step 2 已确认的分支名，不得再次暂停。若旧状态恢复时缺少该次联合决策中的分支名，重新进入 Step 2 的同一个联合决策；不得创建第二个独立分支命名决策点。
  ...
  分支名由 Step 2 确认后，立即执行 `git checkout -b <branch-name>`，然后运行 `node "<comet-state-script>" set <name> isolation branch`，把新分支写入 `bound_branch`。后续工作在新分支上进行。
...
进入最终执行分支或 worktree 后，必须在该实际工作区重新绑定当前 change。branch 模式已在切换后通过 `isolation branch` 绑定；worktree 模式必须在新工作区运行 `node "<comet-state-script>" set <name> isolation worktree`，把 worktree 的当前分支写入 `bound_branch`。新 worktree 不会继承原工作区的本地选择文件，因此还必须选择当前 change：
...
- `build_mode: executing-plans`：**立即执行：** 使用 Skill 工具加载 Superpowers `executing-plans` 技能。禁止跳过此步骤。若该技能不可用，停止流程并提示安装或启用对应技能，不要用普通对话替代该步骤。技能加载后，ARGUMENTS 必须包含与 Step 1 相同的 Language 约束：`Language: 使用 node "<comet-state-script>" get <name> language 读取到的 Comet 配置产物语言输出`。按计划执行。
...
- 如果执行前复检发现后台调度能力已失效，不得直接在主窗口执行，也不得创建新的二次决策；返回 Step 2 的同一个联合决策，移除不可用模式。用户在该联合决策中选择主窗口执行后，先运行 `node "<comet-state-script>" set <name> build_mode executing-plans`，再按对应分支继续。
...
- `requesting-code-review` 技能必须在 `node "<comet-guard-script>" <change-name> build --apply` 之前加载
...
- **阶段守卫**：运行 `node "<comet-guard-script>" <change-name> build --apply`，全部 PASS 后由守卫推进到 `phase: verify`（此步骤更新 `phase` 字段，与 `auto_transition` 无关）
