...
验证报告必须使用 `node "$COMET_STATE" get <name> language` 读取到的 Comet 配置产物语言。
...
先运行 `node "$COMET_STATE" get <change-name> verify_failures` 读取已持久化的连续失败次数。前 3 次可修复失败自动回到 build：报告失败项后运行 `node "$COMET_STATE" transition <change-name> verify-fail`，再调用 `/comet-build` 修复，不需要用户确认。
...
```bash
node "$COMET_STATE" record-check <change-name> verify --command "<实际运行的验证命令>" --exit-code 0
```
...
- **阶段守卫**：运行 `node "$COMET_GUARD" <change-name> verify --apply`，全部 PASS 后由守卫通过 `node "$COMET_STATE" transition verify-pass` 推进到 `phase: archive`（此步骤更新 `phase` 字段，与 `auto_transition` 无关）
