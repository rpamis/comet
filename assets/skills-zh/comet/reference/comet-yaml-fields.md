...
| `isolation` | `current`、`branch` 或 `worktree`。full 初始化可为 `null`，离开 build 前必须实际创建/选择 `branch` 或 `worktree`；hotfix/tweak 在入口用户决策点后可如实使用 `current`、`branch` 或 `worktree`，不得在未创建分支时虚构为 `branch` |
| `bound_branch` | 当前分支绑定记录，可为空。仅 `isolation: current` 使用：首次设置或入口检查时记录当前 Git 分支，后续 `comet state select` / `comet state check` 必须确认绑定分支与当前分支一致；漂移时进入 `BLOCKED`，按决策点协议让用户选择切回绑定分支或明确确认后运行 `comet state rebind <change-name>`。`isolation: branch` / `worktree` 会清空该字段 |
