...
1. 先运行 `git status --short`，记录已提交、已暂存、未暂存和未跟踪内容的边界。
2. Classic 优先使用已经验证为 commit 的 `base_ref`；如果 plan frontmatter 提供有效的 `base-ref`，它必须与状态基线一致，否则报告范围不确定并停止。
3. Native 优先使用状态中已验证的 target/change branch 关系与当前 candidate 的实现范围证据。
4. 查看从可信基线到当前工作树的完整差异，包括已提交、已暂存和未暂存修改。对属于当前 change 的所有未跟踪文件，包括源码、测试、文档、配置和元数据（例如 `SKILL.md` 与 `agents/openai.yaml`），直接读取内容并明确标注其未跟踪状态。
5. 不审查明确归属于其他 change 或用户无关工作的差异；无法可靠归因时报告范围冲突并停止，不猜测归属。

如果没有可验证的基线，只能审查当前可见的工作树差异，并必须在结果中显著标注“审查范围不完整”。
