...
传递给 OpenSpec 的所有提问和产物要求都必须包含解析后的 Comet 产物语言，并使用 `en`、`zh-CN` 这类规范化 ID。`.comet.yaml` 尚不存在时依次读取项目 `.comet/config.yaml` 和全局 `~/.comet/config.yaml` 的 `classic.language`；change 初始化后使用 `node "$COMET_STATE" get <name> language` 读取。没有配置语言时才回退到当前用户请求语言。生成的 `proposal.md`、`design.md`、`tasks.md` 必须以该语言为主语言。
...
- 状态文件缺失但 change 目录有效：先运行 `node "$COMET_STATE" init <change-name> full`，再选择 change
...
1. 状态文件缺失时先运行 `node "$COMET_STATE" init <name> full`；格式异常时停止并修复，不得覆盖。随后选择 change 并运行 `node "$COMET_STATE" check <name> open`。
