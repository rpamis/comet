...
恢复已有 hotfix change 时，第一项状态操作必须是 `node "$COMET_STATE" select <change-name>`；创建新 change 时，在 `.comet.yaml` 初始化成功后立即运行该命令，再进入源码写入步骤。
...
- B. 创建分支：先创建并切换到 `hotfix/YYYYMMDD/<change-name>`，再运行 `node "$COMET_STATE" set <name> isolation branch`
...
hotfix 的升级判定只决定是否从预设流程转为 full；文件数不自动升级，`node "$COMET_STATE" scale` 只决定验证轻重。
