...
```bash
node "<comet-handoff-script>" <change-name> design --write
```
...
```bash
node "<comet-handoff-script>" <change-name> design --write --full
```
...
```bash
# 记录 design_doc 路径
node "<comet-state-script>" set <name> design_doc docs/superpowers/specs/YYYY-MM-DD-topic-design.md

# 如有 delta spec 变更，重新生成 handoff（更新 hash）
node "<comet-handoff-script>" <change-name> design --write

# 阶段守卫推进 phase 到下一阶段
node "<comet-guard-script>" <change-name> design --apply
```
...
- **阶段守卫**：运行 `node "<comet-guard-script>" <change-name> design --apply`，全部 PASS 后由守卫推进到 `phase: build`（此步骤更新 `phase` 字段，与 `auto_transition` 无关）
