...
- 若 `phase: open` 但 OpenSpec `applyRequires` 已完整，先运行 `node "$COMET_GUARD" <change-name> open --apply` 修正状态，再继续判定
...
3. **验证级别**（scale 脚本判定）：`node "$COMET_STATE" scale` 仅决定 `verify_mode`（验证轻重），不卡流程、不触发升级

**升级决策点（用户二选一）**：
- 继续预设轻量流程（用户确认范围可控）
- 升级为完整 `/comet-classic`（使用 `node "$COMET_STATE" transition <name> preset-escalate` 合法回退到 design 阶段，同时清除预设专属的 build 配置；补 Design Doc 后重新联合选择完整工作方式）
...
| `.comet.yaml` 缺失 | 进入对应 preset 的 `/comet-open` 初始化状态，再运行 `node "$COMET_STATE" select`；不得跳过初始化 |
| `.comet.yaml` 格式异常 | 停止并报告解析错误；从版本控制、备份或可验证产物人工修复，不能用 `node "$COMET_STATE" set` 覆盖损坏文件 |
...
- 这些约束同时由 `node "$COMET_GUARD" <name> build --apply` 和 `node "$COMET_STATE" transition <name> build-complete` 执行

### 脚本定位

每个会话按 `comet/reference/scripts.md` 完成一次脚本定位，缓存 `$COMET_STATE`、`$COMET_GUARD`、`$COMET_HANDOFF`、`$COMET_ARCHIVE`（以及入口路由用的 `$COMET_INTENT` 和 `$COMET_RESUME_PROBE`）。直连 bundle 调用比 `comet` CLI 外壳更快。关键入口：
