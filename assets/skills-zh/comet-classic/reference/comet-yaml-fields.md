...
| `build_mode`           | 已选择的执行方式，可为空。`autonomous`：显式选择的自主规划与执行，可使用有界工作包，不强制外部规划/执行 Skill；`subagent-driven-development`：主会话协调后台实现；`executing-plans`：主会话按计划顺序执行；`direct`：默认仅 hotfix/tweak，full 需 direct_override。写计划前确认策略，不按模型名称自动替换旧配置                                                                                                                                                                                                                                               |
| `build_pause`          | build 阶段内部暂停点。`null` 表示无暂停，`plan-ready` 表示 plan 已生成且用户明确要求计划后暂停（包括切换模型）；收到继续指令后沿用有效计划与配置，它本身不要求重新配置                                                                                                                                                                                                                                                                                                                                                                                        |
...
| `branch_status`        | `pending` 或 `handled`。verify 保持 pending；归档时 handled 仅保留兼容语义，不能推断 local/push/pr 授权或成功。实际授权由 state delivery 保存，所选本地/远端动作经 Runtime 核对完成后才清除 selection                                                                                                                                                                                                                                                                                                                                                         |
| `created_at`           | change 创建日期（init 时自动写入），格式 `YYYY-MM-DD`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| `verified_at`          | 验证通过时间，可为空                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| `archive_confirmation` | `null`、`pending` 或 `confirmed`。verify-pass 进入 archive 时写 pending；用户明确选择归档并仅本地、push 或 PR 后，通过 archive-confirm 写 confirmed；archive-reopen 清空确认并使旧 delivery 授权失效，重新验证后再次确认                                                                                                                                                                                                                                                                                                                                      |
...
- full 的 `build_mode: autonomous` 仍须有效 design_doc、plan、isolation、tdd_mode，并选择 standard/thorough 独立审查；它不是 direct override，也不更改已有策略
...
- 协调与交付记录通过 `state checkpoint` / `state delivery` 管理，不把其 JSON 字段直接写入 .comet.yaml；读取和恢复见 context-recovery.md 与 comet-archive
