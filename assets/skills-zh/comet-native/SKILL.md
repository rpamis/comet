...
确认后，Runtime 先创建父级专用 integration workspace（独立 branch/worktree），再从当前 integration HEAD 生成绑定 Child、角色、工作区、基线提交和 `runId` 的任务包。Skill 从 `readyChildren` 自动派发当前可执行 Child；平台支持并行时最多同时派发两个无依赖任务，不支持并行时按顺序执行。子 change 范围严格继承父确认，出现新的用户可见决定时回到父 Shape。
...
确认一次 Supervisor Change Shape 即授权严格派生的 Child，不要求用户重复确认相同范围。Skill 只执行 Runtime continuation 返回的动作，并在每个任务完成后重新读取 `readyChildren`，继续下一波；Child 必须经过 `active → verified → integrated`，只有父级最终 Verify 和交付完成后才统一 `archived`。Supervisor Change 最后仍由 Verify 在 integration workspace 验证完整验收项。
 ...
 状态包含 `children` 时，当前 change 是 Supervisor Change：不要运行 Supervisor Change Builder，只推进 Runtime 返回的 `readyChildren` 和 Supervisor 任务。Runtime 为每个 Child 返回独立 worktree、当前 integration HEAD、角色、任务包和 `runId`；Builder 与 Verifier 回报必须携带当前 `runId`，重复或迟到的回报一律拒绝。无依赖 Child 可以并行 Build/Verify；Skill 不逐个执行 Archive，原 `finish=merge` 边界由 Runtime 完成。只有 Child 经过 `active → verified → integrated` 且完成最小集成检查后才算 `integrated`，Agent 完成消息或未提交文件都不代表集成完成；Skill 每个任务完成后重新读取 `readyChildren`，按 Runtime 顺序继续。
 全部 Child 为 `integrated` 后，立即消费 Runtime 返回的 `parentAdvance`，通知用户父级进入最终 Verify，不要求用户再次说“推进”。最终 Verify 在 integration workspace 检查完整验收项；失败时保留冲突/阻塞现场，不重开已归档或已 integrated Child，按 `repair-child` 在 v2 `acceptance_index` 补充实际失败的 Spec 验收文字并追加唯一 repair child，重新确认 Shape 后继续。target 分支在最终交付前保持不变，最终 Archive、workspace finish、merge、push 和 PR 仍按原授权边界处理。
