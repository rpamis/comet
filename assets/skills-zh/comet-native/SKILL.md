...
先守住这条优先边界：能从环境得到的事实由模型自行调查；只要两个合理答案会产生不同的用户可见结果，它就是用户决定，决定权属于用户，模型只能给出推荐；只有不同做法产生相同用户可见结果时，才是模型自主选择的实现方式。用户决定一次只问一个并等待回答；答案到来前不把任何选项写成既定规格、不调用 `next`、不开始实现。没有用户决定时才自动推进。
...
在判定决策前沿为空前，先做一次很短的“用户可见契约扫描”：把每个关键名词或动作写成一个能区分解释的输入→输出或触发→结果示例，再检查主要分支、默认行为、边界条件、失败路径、兼容性约束和不可逆操作。文本或 token 行为覆盖大小写、首尾与内部标点、空白、Unicode、空输入、重复项、顺序和并列结果；CLI/API 行为覆盖默认值、错误结果和向后兼容。

只有用户已给信息、明确非目标或已确认决定，以及直接覆盖这个精确行为的既有用户可见契约、测试或规则，才能消除一个分支。相邻功能的实现方式、保持一致、行业惯例或最小改动属于仓库事实，只能用于形成推荐项，不能代替用户答案。

一旦一个反例能区分两种合理解释，它就是用户决定：在 brief 保留一个 `[blocking]` 问题，询问最上游、最重要的一个问题，同时给出推荐答案、简短理由和实际影响，然后结束本轮等待回答。回答前不得把任一输出写成已决定的规格、调用 `next` 或开始实现；回答后更新原有产物并重新计算决策前沿，再决定继续或询问下一个问题。
