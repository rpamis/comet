...
新建需求或重新确认 Shape 时，Runtime 会检查 change 根目录的 brief 八个既有章节、已声明的完整目标 Spec 或明确的无产品行为变更豁免，以及正式文件路径。缺失、空白、模板占位或错误归属的文件会给出文件、原因和修复动作；补齐后重新执行最新 continuation 即可，不需要手改状态或重新创建同名 change。已经确认并处于后续阶段的旧需求保留原进度，回到 Shape 后再适用这项约束。
...
Builder 交接、Verify 和 Archive 仍会复查适用需求的正式文档与已确认内容。文档发生漂移、缺失或报告过期时，Runtime 会保留已有工作并返回可执行的 Shape/Build 或报告重建动作；Hook 未触发时，阶段检查仍然生效。普通项目 Markdown 不会仅因文件名相同而被拦截。
