...
进入 Shape 后必须读取本文件。在完成问题判定、检查未明说的假设并取得共同理解前，保持项目实现不变并停留在 Shape。
...
只有歧义会实质改变用户可见结果，而且无法从用户要求、正式规格或项目规则中可靠确定时，才询问用户。问题应来自真实分歧；普通实现选择由 Agent 直接决定。
...
4. 一次提出一个当前可提问节点并等待回答；下一轮再提出下一个用户决定。
