...
只有歧义会实质改变用户可见结果，而且无法从用户要求、正式规格、项目文档、现有 Agent 指令或项目规则中可靠确定时，才询问用户。问题应来自真实分歧；普通实现选择由 Agent 直接决定。
- 用户直接提供文件、附件、链接或本地路径作为需求来源时，先在 `brief.md` 的 `# Scope` 下记录 `## Source coverage`，再在 `# Open questions` 提出歧义、遗漏或隐含边界问题。排错、取证、审查或实现参考材料不自动触发，用途不明时先澄清。
- 按标题、段落、列表、表格、代码块、示例、约束、链接和边界建立来源单元。可执行单元必须同时映射到完整目标 Spec 和至少一个验收 ID；不可访问、无法解析、读取不完整或尚未映射的内容保持 `[blocking]`。分块读取不减少最终覆盖范围，摘要不能代替来源覆盖映射。
