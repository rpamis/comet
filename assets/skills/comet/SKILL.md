...
1. Run the public Comet CLI installed on PATH in the current project:
   ...
   If project config is missing, this snapshots global defaults and creates artifact directories inside the project. Later global changes do not rewrite it.
2. Parse the JSON. Only accept `schema: comet.workflow-resolution.v1` and a `skill` value listed below.
   If it returns `command not found`, the CLI install is incomplete. If the CLI starts but exits nonzero, returns invalid JSON, or reports invalid config, stop with the original error. Do not search for Skill files, scan platform configuration directories, or invoke an internal bundle directly. Never fall back or guess.
