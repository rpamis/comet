...
- **Phase exit**: `node "<comet-guard-script>" <name> <phase> --apply` (must see ALL CHECKS PASSED)
- **Compression recovery**: `node "<comet-state-script>" check <name> <phase> --recover`
- **State update**: After key operations, update fields through `node "<comet-state-script>" set`; manually editing .comet.yaml is prohibited
- **Phase advancement only via guard/transition**: directly running `node "<comet-state-script>" set <name> phase <value>` to jump phases is prohibited. A preset (hotfix/tweak) upgrade to full must use `node "<comet-state-script>" transition <name> preset-escalate`
- **handoff generation**: `node "<comet-handoff-script>" <name> design --write` (handwriting summaries is prohibited)
...
1. First script operation = `node "<comet-handoff-script>" <name> design --write` (loading brainstorming before generating handoff is prohibited)
...
1. First step run `node "<comet-state-script>" scale <name>` to determine verification level
2. For the first 3 clearly repairable failures, automatically run `node "<comet-state-script>" transition <name> verify-fail`, return to build, and enter `/comet-build`. CRITICAL/IMPORTANT failures cannot be accepted as deviations
