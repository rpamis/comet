...
- **阶段退出**: `node "$COMET_GUARD" <name> <phase> --apply`（必须看到 ALL CHECKS PASSED）
- **压缩恢复**: `node "$COMET_STATE" check <name> <phase> --recover`
- **状态更新**: 关键操作后通过 `node "$COMET_STATE" set` 更新字段，禁止手工编辑 .comet.yaml
- **阶段推进只能经 guard/transition**: 禁止用 `node "$COMET_STATE" set <name> phase <值>` 手动跳阶段；预设升级到 full 必须用 `node "$COMET_STATE" transition <name> preset-escalate`
- **handoff 生成**: `node "$COMET_HANDOFF" <name> design --write`（禁止手写摘要）
...
1. 第一个脚本操作 = `node "$COMET_HANDOFF" <name> design --write`（未生成 handoff 禁止加载 brainstorming）
...
1. 第一步运行 `node "$COMET_STATE" scale <name>` 确定验证级别
2. 前 3 次明确可修复失败自动运行 `node "$COMET_STATE" transition <name> verify-fail` 回到 build，并进入 `/comet-build`；CRITICAL/IMPORTANT 不得接受偏差
