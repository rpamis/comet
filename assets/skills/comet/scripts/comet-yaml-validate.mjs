...
function filesystemPath(relativePath) {
...
var classicValidateCommand = async (args) => {
  ...
  const yamlFile = path2.join(directory, ".comet.yaml");
  ...
  let source;
  try {
    source = await fs2.readFile(yamlFile, "utf8");
  } catch (error) {
  ...
  if (document.errors.length > 0 || !(0, import_yaml.isMap)(document.contents)) {
    for (const error of document.errors) fail(error.message);
    if (!(0, import_yaml.isMap)(document.contents)) fail("document root must be a mapping");
    lines.push("", color(RED, `${errors} error(s), ${warnings} warning(s) — validation FAILED`));
    return { exitCode: 1, stderr: lines.join("\n") };
  }
  ...
  for (const field of ["design_doc", "plan", "handoff_context"]) {
    const value = text(record[field]);
    if (value && !await exists2(path2.resolve(value))) {
