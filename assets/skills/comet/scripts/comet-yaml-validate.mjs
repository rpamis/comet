...
var DEFAULT_WORKFLOW_NATIVE_MAX_VERIFY_FAILURES = 5;
...
function classicLayoutPaths(projectRoot, artifactLayout) {
  ...
  const openSpecBase = artifactLayout === "docs" ? path3.join(root, "docs") : root;
  const openSpecRoot = path3.join(openSpecBase, "openspec");
  const superpowersRoot = path3.join(root, "docs", "superpowers");
  return {
    projectRoot: root,
    artifactLayout,
    openSpecBase,
    openSpecRoot,
    changesDir: path3.join(openSpecRoot, "changes"),
    archiveDir: path3.join(openSpecRoot, "changes", "archive"),
    specsDir: path3.join(openSpecRoot, "specs"),
    superpowersRoot,
    superpowersSpecsDir: path3.join(superpowersRoot, "specs"),
    superpowersPlansDir: path3.join(superpowersRoot, "plans"),
    superpowersReportsDir: path3.join(superpowersRoot, "reports")
  };
}
...
async function discoverClassicProject(startPath) {
  let cursor = path3.resolve(startPath);
  let openSpecFallback = null;
  try {
    if (!(await fs4.lstat(cursor)).isDirectory()) cursor = path3.dirname(cursor);
  } catch (error) {
    if (!isMissingPath2(error)) throw error;
  }
  for (; ; ) {
    if (path3.basename(cursor) === "openspec") {
      openSpecFallback = path3.dirname(cursor);
    }
    const configFile = path3.join(cursor, ".comet", "config.yaml");
    ...
    if (projectConfig || await fileExists(path3.join(cursor, ".git"))) {
      return cursor;
    }
    const parent = path3.dirname(cursor);
    if (parent === cursor) return openSpecFallback ?? path3.resolve(startPath);
    cursor = parent;
  }
}
function classicProjectRelative(projectRoot, target) {
  return path3.relative(path3.resolve(projectRoot), target).replaceAll("\\", "/");
}
...
function projectTarget(projectRoot, target, label) {
  ...
  const absolute = path4.isAbsolute(target) ? path4.resolve(target) : path4.resolve(root, target);
  const relative = path4.relative(root, absolute).replaceAll("\\", "/");
  return {
    root,
...
async function inspectChangeDirectory(projectRoot, directory, label) {
  ...
  const state = await inspectClassicProjectTarget(
    projectRoot,
    path5.join(directory, ".comet.yaml"),
    {
      label: `${label} state`,
      expected: "file"
    }
...
async function findArchiveChangeDirectory(projectRoot, archiveDir, name, options = {}) {
  ...
  const matches = [];
  for (const entry of await fs5.readdir(archiveDir, { withFileTypes: true })) {
    if (!archiveNameMatchesChange(entry.name, name)) continue;
    const candidate = await inspectChangeDirectory(
      projectRoot,
      path5.join(archiveDir, entry.name),
...
async function resolveClassicChangeDirectory(name, projectRoot = process.cwd()) {
  ...
  const active = await inspectChangeDirectory(
    layout.projectRoot,
    path5.join(layout.changesDir, name),
...
var RUN_STATE_MAX_BYTES = 256 * 1024;
...
async function resolveCommandContext(options) {
  const invocationCwd = path6.resolve(options.invocationCwd ?? process.cwd());
  const projectRoot = path6.resolve(
  ...
  if (!isInside2(realProjectRoot, realInvocationCwd)) {
    throw new Error(
...
var classicValidateCommand = async (args, options) => withClassicCommandContext(options, async () => {
  ...
  const yamlFile = path7.join(directory, ".comet.yaml");
  ...
  if (document.errors.length > 0 || !(0, import_yaml2.isMap)(document.contents)) {
    for (const error of document.errors) fail(error.message);
    if (!(0, import_yaml2.isMap)(document.contents)) fail("document root must be a mapping");
  ...
  for (const field of ["design_doc", "plan", "handoff_context", "verification_report"]) {
    const value = text(record[field]);
    if (!value) continue;
    ...
    try {
      if (!await classicProjectTargetExists(projectRoot, path7.resolve(projectRoot, value), {
  ...
  for (const field of ["handoff_hash"]) {
    const value = text(record[field]);
  ...
  for (const field of Object.keys(record)) {
    if (!KNOWN_KEYS2.has(field)) warn(`unknown field '${field}' found`);
  }
  lines.push("");
  if (errors > 0) {
    lines.push(color(RED, `${errors} error(s), ${warnings} warning(s) — validation FAILED`));
...
// domains/comet-classic/classic-validate-entry.ts
