...
import { promises as fs9, readFileSync } from "fs";
...
async function nonempty(file) {
  try {
    return (await fs9.stat(file)).size > 0;
  } catch (error) {
    if (error.code === "ENOENT") return false;
    throw error;
  }
...
async function handoffSourceFiles(changeDir) {
  ...
  if (await exists(specs)) {
    for (const entry of (await fs9.readdir(specs)).sort()) {
...
async function writeMarkdownContext(changeDir, change, mode, contextHash, output) {
  ...
  await fs9.writeFile(output, lines.join("\n"));
}
async function writeJsonContext(changeDir, change, mode, contextHash, output) {
  ...
  await fs9.writeFile(output, document);
}
...
async function writeSpecMarkdownContext(changeDir, change, contextHash, output) {
  ...
  if (await exists(specs)) {
    for (const entry of (await fs9.readdir(specs)).sort()) {
      ...
      projected = true;
      lines.push(...await writeSpecProjectionForFile(spec, await fs9.readFile(spec, "utf8")));
    }
  }
  if (!projected) {
...
async function readField(changeDir, field2) {
  ...
  const document = (0, import_yaml3.parseDocument)(await fs9.readFile(file, "utf8"), { uniqueKeys: false });
  if (document.errors.length > 0) {
...
async function completedHandoffIsCurrent(changeDir, run, contextHash, contextJson, contextMd) {
  ...
  if (context !== await fs9.readFile(contextMd, "utf8")) return false;
...
var classicHandoffCommand = async (args) => {
  ...
  try {
    ...
    await fs9.mkdir(handoffDir, { recursive: true });
    if (handoffMode === "beta") {
    ...
    const context = await fs9.readFile(contextMd, "utf8");
