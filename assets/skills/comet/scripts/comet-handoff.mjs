...
function projectTarget(projectRoot, target, label) {
  ...
  const absolute = path5.isAbsolute(target) ? path5.resolve(target) : path5.resolve(root, target);
  const relative = path5.relative(root, absolute).replaceAll("\\", "/");
  return {
    root,
...
async function inspectChangeDirectory(projectRoot, directory, label) {
  ...
  const state = await inspectClassicProjectTarget(
    projectRoot,
    path6.join(directory, ".comet.yaml"),
    {
      label: `${label} state`,
      expected: "file"
    }
...
async function inspectClassicActiveChangeDirectory(name, projectRoot = process.cwd()) {
  ...
  const inspection = await inspectChangeDirectory(
    layout.projectRoot,
    path6.join(layout.changesDir, name),
...
async function readLegacyArchivedHandoffFallback(projectRoot, changeDir, pointer, maxBytes) {
  ...
  const archivedChange = path7.resolve(changeDir);
  if (path7.dirname(archivedChange) !== path7.resolve(layout.archiveDir)) return null;
  const archivedName = path7.basename(archivedChange);
  ...
  const mapped = path7.join(archivedChange, ".comet", ...suffix);
...
function relativeSource(projectRoot, file) {
  return path8.relative(projectRoot, file).split(path8.sep).join("/");
}
...
async function deltaSpecEvidence(projectRoot, changeDir) {
  const specsDir = path8.join(changeDir, "specs");
  let entries;
  try {
    ...
    entries = await fs6.readdir(specsDir);
  } catch (error) {
  ...
  const candidates = entries.map((entry) => path8.join(specsDir, entry, "spec.md"));
...
async function collectClassicEvidence(changeDir, projection) {
  ...
  const proposal = path8.join(changeDir, "proposal.md");
  const design = path8.join(changeDir, "design.md");
  const tasks = path8.join(changeDir, "tasks.md");
  const checkpoint = projection.run ? path8.resolve(changeDir, projection.run.checkpointRef) : path8.join(changeDir, ".comet", "checkpoint.json");
...
function assertRunPath(changeDir, relativePath2) {
  if (path11.isAbsolute(relativePath2)) {
    throw new Error("Run path must stay inside the change directory");
  }
  const root = path11.resolve(changeDir);
  const target = path11.resolve(root, relativePath2);
...
async function readRuntimeChecks(cometRoot) {
  const checksPath = path12.join(cometRoot, "checks.yaml");
  const evalsPath = path12.join(cometRoot, "evals.yaml");
...
async function loadPackageFromLayout(options) {
  const packageRoot = path12.resolve(options.root);
  const controlRoot = path12.resolve(options.controlRoot);
  if (options.requireSkillMarkdown) {
    await fs7.access(path12.join(packageRoot, "SKILL.md"));
  }
  const skillPath = path12.join(controlRoot, "skill.yaml");
  const guardrailsPath = path12.join(controlRoot, "guardrails.yaml");
...
async function loadSkillPackage(root) {
  const packageRoot = path12.resolve(root);
  return loadPackageFromLayout({
    root: packageRoot,
    controlRoot: path12.join(packageRoot, "comet"),
    packageKind: "skill",
...
function loadSkillPackageDocument(document, root, filePath = path12.join(root, "package.json")) {
  assertObject(document, filePath);
...
function validateSkillPackage(pkg) {
  ...
  for (const tool of definition.tools) {
    ...
    const normalized = path13.posix.normalize(tool.source.replaceAll("\\", "/"));
    if (path13.isAbsolute(tool.source) || normalized === ".." || normalized.startsWith("../")) {
...
function normalizedRelativePath(source) {
  return path14.posix.normalize(source.replaceAll("\\", "/"));
}
function assertInside(parent, target, label) {
  const relative = path14.relative(parent, target);
  if (relative === "" || !path14.isAbsolute(relative) && !relative.startsWith(`..${path14.sep}`)) {
    return;
  }
...
async function readPackageFile(root, relativePath2, label) {
  ...
  if (path14.posix.isAbsolute(normalized) || normalized === ".." || normalized.startsWith("../")) {
    throw new Error(`${label} resolves outside the Skill package`);
  }
  const target = path14.resolve(root, ...normalized.split("/"));
  assertInside(root, target, label);
  let realTarget;
  try {
    realTarget = await fs8.realpath(target);
  } catch (error) {
    if (error.code === "ENOENT") {
  ...
  if (!(await fs8.stat(realTarget)).isFile()) {
    throw new Error(`${label} is not a file: ${relativePath2}`);
  }
  return { path: normalized, content: await fs8.readFile(realTarget) };
}
async function snapshotFiles(pkg) {
  const root = await fs8.realpath(pkg.root);
...
async function verifyPublishedSnapshot(snapshotDir, material) {
  try {
    const storedHash = (await fs8.readFile(path14.join(snapshotDir, "sha256"), "utf8")).trim();
    if (storedHash !== material.hash) throw new Error("hash mismatch");
    const storedPackage = await fs8.readFile(path14.join(snapshotDir, "package.json"), "utf8");
    if (storedPackage !== packageJson(material.document)) throw new Error("package mismatch");
    for (const file of material.files) {
      const stored = await fs8.readFile(path14.join(snapshotDir, ...file.path.split("/")));
...
async function createSkillSnapshot(pkg, changeDir) {
  ...
  const snapshotsRoot = path14.resolve(changeDir, ".comet", "skill-snapshots");
  const snapshotDir = path14.join(snapshotsRoot, material.hash);
  await fs8.mkdir(snapshotsRoot, { recursive: true });
  if (await pathExists(snapshotDir)) {
    await verifyPublishedSnapshot(snapshotDir, material);
    return { hash: material.hash, snapshotDir };
  }
  const temporaryDir = path14.join(snapshotsRoot, `.tmp-${randomUUID2()}`);
  assertInside(snapshotsRoot, temporaryDir, "Temporary snapshot");
  assertInside(snapshotsRoot, snapshotDir, "Published snapshot");
  try {
    await fs8.mkdir(temporaryDir);
    for (const file of material.files) {
      const destination = path14.join(temporaryDir, ...file.path.split("/"));
      assertInside(temporaryDir, destination, `Snapshot file ${file.path}`);
      await fs8.mkdir(path14.dirname(destination), { recursive: true });
      await fs8.writeFile(destination, file.content);
    }
    await fs8.writeFile(path14.join(temporaryDir, "package.json"), packageJson(material.document));
    await fs8.writeFile(path14.join(temporaryDir, "sha256"), material.hash + "\n");
    await fs8.rename(temporaryDir, snapshotDir);
  } catch (error) {
    if (await pathExists(snapshotDir)) {
      try {
        await verifyPublishedSnapshot(snapshotDir, material);
      } finally {
        await fs8.rm(temporaryDir, { recursive: true, force: true });
      }
      return { hash: material.hash, snapshotDir };
    }
    await fs8.rm(temporaryDir, { recursive: true, force: true });
...
async function readSkillSnapshot(changeDir, hash) {
  ...
  const snapshotsRoot = path14.resolve(changeDir, ".comet", "skill-snapshots");
  const snapshotDir = path14.join(snapshotsRoot, hash);
  ...
  try {
    ...
    const packagePath = path14.join(snapshotDir, "package.json");
    const document = JSON.parse(await fs8.readFile(packagePath, "utf8"));
...
async function migrationArtifacts(changeDir, evidence) {
  ...
  const progress = path15.join(changeDir, "subagent-progress.md");
  if (await pathExists2(progress)) {
    artifacts.subagent_progress = path15.relative(projectRoot, progress).split(path15.sep).join("/");
  }
...
async function removeCreatedFiles(files) {
  await Promise.all(files.map((file) => fs9.rm(file, { recursive: true, force: true })));
}
async function ensureClassicRun(changeDir, options) {
  ...
  if (projection.run) {
    ...
    if (installedHash !== projection.run.skillHash) {
      ...
      return {
        ...
        migrated: false,
        snapshotDir: path15.join(changeDir, ".comet", "skill-snapshots", projection.run.skillHash)
      };
    }
  ...
  const expectedSnapshotDir = path15.join(changeDir, ".comet", "skill-snapshots", expectedHash);
  ...
  try {
    ...
    if (handoff?.source) {
      ...
      createdFiles.push(path15.resolve(changeDir, run.contextRef));
    }
    await writeArtifacts(changeDir, run.artifactsRef, artifacts);
    createdFiles.push(path15.resolve(changeDir, run.artifactsRef));
    ...
    createdFiles.push(path15.resolve(changeDir, run.checkpointRef));
    createdFiles.push(path15.resolve(changeDir, run.trajectoryRef));
    ...
    if (!snapshotExisted) await fs9.rm(expectedSnapshotDir, { recursive: true, force: true });
    throw error;
  }
}
...
async function directoryExists(directory) {
  try {
    return (await fs10.stat(directory)).isDirectory();
  } catch (error) {
    if (error.code === "ENOENT") return false;
    throw error;
  }
...
async function isClassicRuntimePackageRoot(root) {
  ...
  if (await fileExists2(path16.join(root, "skill.yaml"))) return true;
  return await fileExists2(path16.join(root, "SKILL.md")) && await fileExists2(path16.join(root, "comet", "skill.yaml"));
}
...
async function classicRuntimeRoot() {
  const runtimeDirectory = path16.dirname(fileURLToPath(import.meta.url));
  const candidates = [
    process.env.COMET_RUNTIME_CLASSIC_ROOT,
    path16.resolve(runtimeDirectory, "..", "runtime", "classic"),
    path16.resolve(runtimeDirectory, "..", "..", "comet", "runtime", "classic"),
    path16.resolve(runtimeDirectory, "..", "..", "assets", "skills", "comet", "runtime", "classic"),
    path16.resolve("assets", "skills", "comet", "runtime", "classic"),
    process.env.COMET_CLASSIC_SKILL_ROOT,
    path16.resolve(runtimeDirectory, "..", "..", "comet-classic"),
    path16.resolve(runtimeDirectory, "..", "..", "assets", "skills", "comet-classic"),
    path16.resolve("assets", "skills", "comet-classic")
...
async function loadClassicRuntimePackage(root) {
  if (await fileExists2(path16.join(root, "skill.yaml"))) {
...
async function ensureClassicRuntimeRun(changeDir) {
  ...
  return ensureClassicRun(changeDir, {
    skillPackage: root ? await loadClassicRuntimePackage(root) : embeddedClassicRuntimePackage(path16.dirname(fileURLToPath(import.meta.url)))
  });
}
...
async function resolveCommandContext(options) {
  const invocationCwd = path17.resolve(options.invocationCwd ?? process.cwd());
  const projectRoot = path17.resolve(
    options.projectRoot ?? await discoverClassicProject(invocationCwd)
  );
  const [realInvocationCwd, realProjectRoot] = await Promise.all([
    fs11.realpath(invocationCwd),
    fs11.realpath(projectRoot)
  ]);
...
async function handoffSourceFiles(projectRoot, changeDir) {
  ...
  if (specsInspection.exists) {
    const entries = (await fs12.readdir(specs, { withFileTypes: true })).sort(
...
function handoffSourceReference(changeDir, changeRef, file) {
  const relative = path18.relative(changeDir, file).replaceAll("\\", "/");
...
async function readField(projectRoot, changeDir, field2) {
  const file = path18.join(changeDir, ".comet.yaml");
  const document = (0, import_yaml4.parseDocument)(
...
async function completedHandoffIsCurrent(projectRoot, changeDir, run, contextHash, contextJson, contextMd, contextJsonRef, contextMdRef) {
  ...
  return checkpoint?.runId === run.runId && checkpoint.contextHash === (context === null ? null : hashText(context)) && checkpoint.artifactsHash === artifactsHash(artifacts) && contextHash.length === 64;
}
var classicHandoffCommand = async (args, options) => withClassicCommandContext(options, async () => {
  ...
  try {
    ...
    const contextCompression = await readField(layout.projectRoot, changeDir, "context_compression") || "off";
    let contextJson;
    let contextMd;
...
// domains/comet-classic/classic-handoff-entry.ts
