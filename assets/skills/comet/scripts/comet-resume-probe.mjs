...
var require_cst_visit = __commonJS({
  "node_modules/.pnpm/yaml@2.9.0/node_modules/yaml/dist/parse/cst-visit.js"(exports) {
    ...
    visit.itemAtPath = (cst, path10) => {
      let item = cst;
      for (const [field2, index] of path10) {
    ...
    visit.parentCollection = (cst, path10) => {
      ...
      const field2 = path10[path10.length - 1][0];
      ...
      for (const field2 of ["key", "value"]) {
        ...
        if (token && "items" in token) {
          for (let i = 0; i < token.items.length; ++i) {
            const ci = _visit(Object.freeze(path10.concat([[field2, i]])), token.items[i], visitor);
            if (typeof ci === "number")
              i = ci - 1;
            else if (ci === BREAK)
              return BREAK;
...
async function inspectProtectedProjectPath(projectRoot, relativePath2, options) {
  ...
  const result3 = await inspectExistingChain(lexicalRoot, realRoot, segments, options);
  return {
...
function decodeRunText(result3, label) {
  try {
    return new TextDecoder("utf-8", { fatal: true }).decode(result3.bytes);
  } catch (error) {
...
async function readSnapshot(changeDir, relativePath2, maxBytes, label, options = {}) {
  try {
    const result3 = await readProtectedProjectFile(changeDir, relativePath2, maxBytes, {
      label,
      hooks: options.hooks
    });
    return { result: result3, text: decodeRunText(result3, label) };
  } catch (error) {
    const code = error.code;
...
function requiredRunReference(doc, key) {
  ...
  if (path5.isAbsolute(value) || /^(?:[A-Za-z]:|[\\/]|~)/u.test(value) || value.split(/[\\/]/u).includes("..")) {
...
async function readClassicState(changeDir, options = {}) {
  ...
  const file = path6.join(changeDir, ".comet.yaml");
...
function result(action, change, confidence, reason, evidence = []) {
  return {
    schema_version: COMET_RESUME_PROBE_SCHEMA_VERSION,
    action,
...
async function resolveCometResumeProbe(projectRoot, rawInput) {
  ...
  if (changes.length > 1) {
    ...
    return hasDecisionPoint(named) ? result("ask_user", named, "low", "active change is at a decision point") : result("auto_resume", named, "high", "request names an active change", [
...
async function resolveCommandContext(options) {
  const invocationCwd = path9.resolve(options.invocationCwd ?? process.cwd());
  const projectRoot = path9.resolve(
    options.projectRoot ?? await discoverClassicProject(invocationCwd)
  );
  const [realInvocationCwd, realProjectRoot] = await Promise.all([
    fs6.realpath(invocationCwd),
    fs6.realpath(projectRoot)
  ]);
...
`);
    ...
    return result2(1, void 0, error instanceof Error ? error.message : String(error));
  }
});

// domains/comet-classic/classic-script-entry.ts
function jsonResult(result3) {
  return {
...
async function runClassicScript(handler, argv = process.argv.slice(2)) {
  ...
  let result3;
  try {
    result3 = await handler(args, { json });
  } catch (error) {
    result3 = {
      exitCode: 70,
  ...
  const output = json ? jsonResult(result3) : result3;
...
// domains/comet-classic/classic-resume-probe-entry.ts
