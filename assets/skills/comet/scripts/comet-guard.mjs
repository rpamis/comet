...
async function linkedFileEvidence(projectRoot, code, relativePath2) {
  ...
  const file = path.resolve(projectRoot, relativePath2);
  return {
    code,
...
async function collectClassicEvidence(changeDir, projection) {
  ...
  const checkpoint = projection.run ? path.resolve(changeDir, projection.run.checkpointRef) : path.join(changeDir, ".comet", "checkpoint.json");
...
async function readRuntimeChecks(cometRoot) {
  const checksPath = path5.join(cometRoot, "checks.yaml");
...
async function loadPackageFromLayout(options) {
  const packageRoot = path5.resolve(options.root);
  const controlRoot = path5.resolve(options.controlRoot);
  if (options.requireSkillMarkdown) {
    await fs5.access(path5.join(packageRoot, "SKILL.md"));
  }
  const skillPath = path5.join(controlRoot, "skill.yaml");
  const guardrailsPath = path5.join(controlRoot, "guardrails.yaml");
...
async function loadSkillPackage(root) {
  ...
  return loadPackageFromLayout({
    root: packageRoot,
    controlRoot: path5.join(packageRoot, "comet"),
    packageKind: "skill",
...
function loadSkillPackageDocument(document, root, filePath = path5.join(root, "package.json")) {
  assertObject(document, filePath);
...
function validateSkillPackage(pkg) {
  ...
  for (const tool of definition.tools) {
    ...
    const normalized = path6.posix.normalize(tool.source.replaceAll("\\", "/"));
    if (path6.isAbsolute(tool.source) || normalized === ".." || normalized.startsWith("../")) {
...
function normalizedRelativePath(source) {
  return path7.posix.normalize(source.replaceAll("\\", "/"));
}
function assertInside(parent, target, label) {
  const relative = path7.relative(parent, target);
  if (relative === "" || !path7.isAbsolute(relative) && !relative.startsWith(`..${path7.sep}`)) {
    return;
  }
...
async function readPackageFile(root, relativePath2, label) {
  ...
  if (path7.posix.isAbsolute(normalized) || normalized === ".." || normalized.startsWith("../")) {
    throw new Error(`${label} resolves outside the Skill package`);
  }
  const target = path7.resolve(root, ...normalized.split("/"));
  ...
  return { path: normalized, content: await fs6.readFile(realTarget) };
}
...
async function verifyPublishedSnapshot(snapshotDir, material) {
  try {
    const storedHash = (await fs6.readFile(path7.join(snapshotDir, "sha256"), "utf8")).trim();
    if (storedHash !== material.hash) throw new Error("hash mismatch");
    const storedPackage = await fs6.readFile(path7.join(snapshotDir, "package.json"), "utf8");
    if (storedPackage !== packageJson(material.document)) throw new Error("package mismatch");
    for (const file of material.files) {
      const stored = await fs6.readFile(path7.join(snapshotDir, ...file.path.split("/")));
...
async function createSkillSnapshot(pkg, changeDir) {
  ...
  const snapshotsRoot = path7.resolve(changeDir, ".comet", "skill-snapshots");
  const snapshotDir = path7.join(snapshotsRoot, material.hash);
  ...
  const temporaryDir = path7.join(snapshotsRoot, `.tmp-${randomUUID4()}`);
  ...
  try {
    ...
    for (const file of material.files) {
      const destination = path7.join(temporaryDir, ...file.path.split("/"));
      assertInside(temporaryDir, destination, `Snapshot file ${file.path}`);
      await fs6.mkdir(path7.dirname(destination), { recursive: true });
      await fs6.writeFile(destination, file.content);
    }
    await fs6.writeFile(path7.join(temporaryDir, "package.json"), packageJson(material.document));
    await fs6.writeFile(path7.join(temporaryDir, "sha256"), material.hash + "\n");
...
async function readSkillSnapshot(changeDir, hash) {
  ...
  const snapshotsRoot = path7.resolve(changeDir, ".comet", "skill-snapshots");
  const snapshotDir = path7.join(snapshotsRoot, hash);
  ...
  try {
    ...
    const packagePath = path7.join(snapshotDir, "package.json");
    const document = JSON.parse(await fs6.readFile(packagePath, "utf8"));
...
function projectRootFor2(changeDir) {
  let cursor = path8.resolve(changeDir);
  while (path8.dirname(cursor) !== cursor) {
    if (path8.basename(cursor) === "openspec") return path8.dirname(cursor);
    cursor = path8.dirname(cursor);
  }
  throw new Error(`Classic change is not inside an openspec directory: ${changeDir}`);
}
...
async function migrationArtifacts(changeDir, evidence) {
  ...
  const progress = path8.join(changeDir, "subagent-progress.md");
  if (await pathExists2(progress)) {
    artifacts.subagent_progress = path8.relative(projectRoot, progress).split(path8.sep).join("/");
  }
...
async function ensureClassicRun(changeDir, options) {
  ...
  if (projection.run) {
    ...
    if (installedHash !== projection.run.skillHash) {
      ...
      return {
        ...
        migrated: false,
        snapshotDir: path8.join(changeDir, ".comet", "skill-snapshots", projection.run.skillHash)
      };
    }
  ...
  const expectedSnapshotDir = path8.join(changeDir, ".comet", "skill-snapshots", expectedHash);
  ...
  try {
    ...
    let context = null;
    if (handoff?.source) {
      context = await fs7.readFile(path8.resolve(projectRoot, handoff.source), "utf8");
      await writeContext(changeDir, run.contextRef, context);
      createdFiles.push(path8.resolve(changeDir, run.contextRef));
    }
    await writeArtifacts(changeDir, run.artifactsRef, artifacts);
    createdFiles.push(path8.resolve(changeDir, run.artifactsRef));
    ...
    createdFiles.push(path8.resolve(changeDir, run.checkpointRef));
    createdFiles.push(path8.resolve(changeDir, run.trajectoryRef));
...
async function classicRuntimeRoot() {
  const runtimeDirectory = path9.dirname(fileURLToPath(import.meta.url));
  const candidates = [
    process.env.COMET_RUNTIME_CLASSIC_ROOT,
    path9.resolve(runtimeDirectory, "..", "runtime", "classic"),
    path9.resolve(runtimeDirectory, "..", "..", "comet", "runtime", "classic"),
    path9.resolve(runtimeDirectory, "..", "..", "assets", "skills", "comet", "runtime", "classic"),
    path9.resolve("assets", "skills", "comet", "runtime", "classic"),
    process.env.COMET_CLASSIC_SKILL_ROOT,
    path9.resolve(runtimeDirectory, "..", "..", "comet-classic"),
    path9.resolve(runtimeDirectory, "..", "..", "assets", "skills", "comet-classic"),
    path9.resolve("assets", "skills", "comet-classic")
...
async function loadClassicRuntimePackage(root) {
  if (await fileExists2(path9.join(root, "skill.yaml"))) {
...
function filesystemPath(relativePath2) {
  return path10.resolve(...relativePath2.split("/"));
}
...
async function resolveClassicChangeDirectory(name) {
  ...
  if (await exists(filesystemPath(archiveRoot))) {
    const matches = [];
    for (const entry of await fs9.readdir(filesystemPath(archiveRoot), { withFileTypes: true })) {
      if (!entry.isDirectory() || !entry.name.endsWith(`-${name}`)) continue;
      const candidate = `${archiveRoot}/${entry.name}`;
      if (await exists(path10.join(filesystemPath(candidate), ".comet.yaml"))) {
...
var classicValidateCommand = async (args) => {
  ...
  const yamlFile = path12.join(directory, ".comet.yaml");
  ...
  let source;
  try {
    source = await fs11.readFile(yamlFile, "utf8");
  } catch (error) {
    if (error.code === "ENOENT") {
  ...
  if (document.errors.length > 0 || !(0, import_yaml3.isMap)(document.contents)) {
    ...
    if (!(0, import_yaml3.isMap)(document.contents)) fail2("document root must be a mapping");
    lines.push("", color(RED, `${errors} error(s), ${warnings} warning(s) — validation FAILED`));
    return { exitCode: 1, stderr: lines.join("\n") };
  }
  ...
  for (const field2 of ["design_doc", "plan", "handoff_context"]) {
    const value = text(record[field2]);
    if (value && !await exists2(path12.resolve(value))) {
...
async function readField(changeDir, field2) {
  ...
  const document = (0, import_yaml4.parseDocument)(await fs12.readFile(file, "utf8"), { uniqueKeys: false });
  if (document.errors.length > 0) {
...
async function projectConfigValue(field2, changeDir) {
  ...
  for (const config of [
    ...
    for (const line of (await fs12.readFile(config, "utf8")).split(/\r?\n/u)) {
...
async function documentLanguageMatchesConfigured(changeDir, file) {
  ...
  const source = stripFencedCodeBlocks(await fs12.readFile(file, "utf8"));
...
${red(
  ...
  for (const part of split) {
    ...
    const result = spawnSync(segment, { shell: true, encoding: "utf8", timeout: 3e5 });
...
function hashFile(file) {
  return createHash3("sha256").update(readFileSync(file)).digest("hex");
}
async function handoffSourceFiles(changeDir) {
  ...
  if (await exists3(specs)) {
    for (const entry of (await fs12.readdir(specs)).sort()) {
...
async function preflight(changeDir, name) {
  if (!await exists3(changeDir)) {
    throw new GuardFailure(red(`FATAL: change directory not found: ${changeDir}`));
  }
  if (!await exists3(path13.join(changeDir, ".comet.yaml"))) {
...
function runInferred(command) {
  const result = spawnSync(command, { shell: true, encoding: "utf8", timeout: 3e5 });
  return {
...
async function buildPasses(changeDir) {
  ...
  if (await exists3("package.json") && /"build"/u.test(await fs12.readFile("package.json", "utf8"))) {
...
async function tasksAllDone(changeDir) {
  const tasks = path13.join(changeDir, "tasks.md");
  if (!await exists3(tasks)) {
    return fail(
      `tasks.md is missing at ${tasks}
Next: restore or create tasks.md for this change before leaving build.`
    );
  }
  const source = await fs12.readFile(tasks, "utf8");
  if (!/- \[x\]/u.test(source)) {
    return fail(
...
async function designDocFrontmatterHas(designDoc, field2, expected) {
  const source = (await fs12.readFile(designDoc, "utf8")).replace(/^\uFEFF/u, "");
...
async function designHandoffMarkdownTraceable(changeDir) {
  ...
  const source = await fs12.readFile(markdown, "utf8");
  const problems = [];
...
async function betaSpecJsonStructurallyValid(changeDir) {
  ...
  const source = await fs12.readFile(context, "utf8");
  const problems = [];
  let parsed;
  try {
...
async function guardOpenChecks(output, changeDir) {
  ...
  const checks = [
    check(
      ...
      async () => await nonempty(path13.join(changeDir, "proposal.md")) ? pass() : fail("")
    ),
    check(
      "proposal.md matches configured language",
      () => documentLanguageMatchesConfigured(changeDir, path13.join(changeDir, "proposal.md"))
    ),
    check(
      "tasks.md exists and non-empty",
      async () => await nonempty(path13.join(changeDir, "tasks.md")) ? pass() : fail("")
    ),
    check(
      "tasks.md matches configured language",
      () => documentLanguageMatchesConfigured(changeDir, path13.join(changeDir, "tasks.md"))
    ),
    check(
  ...
  if (workflow === "full") {
    checks.splice(
      ...
      check(
        ...
        async () => await nonempty(path13.join(changeDir, "design.md")) ? pass() : fail("")
      ),
      check(
        "design.md matches configured language",
        () => documentLanguageMatchesConfigured(changeDir, path13.join(changeDir, "design.md"))
      )
    );
  }
...
async function guardBuildChecks(output, changeDir, change) {
  return runChecks(output, [
    ...
    check("Superpowers plan matches configured language", async () => {
      const plan = await readField(changeDir, "plan");
      if (!plan || plan === "null" || !await exists3(plan)) return pass();
...
async function guardVerifyChecks(output, changeDir) {
  return runChecks(output, [
    ...
    check("verification_report matches configured language", async () => {
      const report = await readField(changeDir, "verification_report");
      if (!report || report === "null" || !await exists3(report)) return pass();
