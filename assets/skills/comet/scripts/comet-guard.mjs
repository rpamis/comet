...
async function atomicWriteContained(file, content, options) {
  ...
  try {
    ...
    if (!temporaryPathStat.isFile() || temporaryPathStat.isSymbolicLink() || !sameFileIdentity(temporaryIdentity, temporaryPathStat) || !isInside(directoryChain[0].realPath, temporaryRealPath)) {
...
async function removeContainedFile(file, options) {
  ...
  if (!identity.isFile() || identity.isSymbolicLink() || !isInside(directoryChain[0].realPath, realPath)) {
...
function projectRelativeSegments(value, label) {
  ...
  if (trimmed.length === 0 || path2.posix.isAbsolute(trimmed) || path2.win32.isAbsolute(trimmed) || /^(?:~|[\\/])/u.test(trimmed)) {
...
var import_yaml, WORKFLOW_PROJECT_CONFIG_MAX_BYTES, MAX_WORKFLOW_SNAPSHOT_PATTERN_LENGTH, MAX_WORKFLOW_SNAPSHOT_PATTERN_WILDCARDS, DEFAULT_WORKFLOW_NATIVE_MAX_VERIFY_FAILURES, DEFAULT_WORKFLOW_NATIVE_SNAPSHOT_CONFIG;
...
async function inspectExistingChain(lexicalRoot, realRoot, segments, options) {
  ...
  for (let index = 0; index < segments.length; index++) {
    ...
    if (!isInside2(realRoot, realCursor)) {
...
async function inspectProtectedProjectPath(projectRoot, relativePath2, options) {
  ...
  if (!isInside2(lexicalRoot, target)) {
...
async function readProtectedProjectFile(projectRoot, relativePath2, maxBytes, options) {
  ...
  return readFileRaceSafe(inspection.target, maxBytes, {
    ...
    verify: async (_checkpoint, context) => {
      if (!isInside2(realRoot, context.realPath)) {
        throw new Error(`${options.label} resolves outside the project root`);
      }
...
function assertRunPath(changeDir, relativePath2) {
  ...
  const root = path4.resolve(changeDir);
  const target = path4.resolve(root, relativePath2);
...
function normalizedCwd(projectRoot, cwd = ".") {
  ...
  const target = path5.resolve(root, cwd);
  ...
  return path5.relative(root, target).replaceAll("\\", "/") || ".";
}
...
async function discoverClassicProject(startPath) {
  ...
  try {
    if (!(await fs5.lstat(cursor)).isDirectory()) cursor = path6.dirname(cursor);
  } catch (error) {
...
async function findArchiveChangeDirectory(projectRoot, archiveDir, name, options = {}) {
  ...
  const matches = [];
  for (const entry of await fs6.readdir(archiveDir, { withFileTypes: true })) {
...
async function deltaSpecEvidence(projectRoot, changeDir) {
  ...
  const candidates = entries.map((entry) => path10.join(specsDir, entry, "spec.md"));
...
async function readRuntimeChecks(cometRoot) {
  const checksPath = path13.join(cometRoot, "checks.yaml");
  const evalsPath = path13.join(cometRoot, "evals.yaml");
...
async function loadPackageFromLayout(options) {
  const packageRoot = path13.resolve(options.root);
  const controlRoot = path13.resolve(options.controlRoot);
  if (options.requireSkillMarkdown) {
    await fs8.access(path13.join(packageRoot, "SKILL.md"));
  }
  const skillPath = path13.join(controlRoot, "skill.yaml");
  const guardrailsPath = path13.join(controlRoot, "guardrails.yaml");
...
async function loadSkillPackage(root) {
  const packageRoot = path13.resolve(root);
  return loadPackageFromLayout({
    root: packageRoot,
    controlRoot: path13.join(packageRoot, "comet"),
    packageKind: "skill",
...
function loadSkillPackageDocument(document, root, filePath = path13.join(root, "package.json")) {
  assertObject(document, filePath);
...
function validateSkillPackage(pkg) {
  ...
  for (const tool of definition.tools) {
    ...
    const normalized = path14.posix.normalize(tool.source.replaceAll("\\", "/"));
    if (path14.isAbsolute(tool.source) || normalized === ".." || normalized.startsWith("../")) {
...
function normalizedRelativePath(source) {
  return path15.posix.normalize(source.replaceAll("\\", "/"));
}
function assertInside(parent, target, label) {
  const relative = path15.relative(parent, target);
  if (relative === "" || !path15.isAbsolute(relative) && !relative.startsWith(`..${path15.sep}`)) {
    return;
  }
...
async function readPackageFile(root, relativePath2, label) {
  ...
  if (path15.posix.isAbsolute(normalized) || normalized === ".." || normalized.startsWith("../")) {
    throw new Error(`${label} resolves outside the Skill package`);
  }
  const target = path15.resolve(root, ...normalized.split("/"));
  ...
  return { path: normalized, content: await fs9.readFile(realTarget) };
}
...
async function verifyPublishedSnapshot(snapshotDir, material) {
  try {
    const storedHash = (await fs9.readFile(path15.join(snapshotDir, "sha256"), "utf8")).trim();
    if (storedHash !== material.hash) throw new Error("hash mismatch");
    const storedPackage = await fs9.readFile(path15.join(snapshotDir, "package.json"), "utf8");
    if (storedPackage !== packageJson(material.document)) throw new Error("package mismatch");
    for (const file of material.files) {
      const stored = await fs9.readFile(path15.join(snapshotDir, ...file.path.split("/")));
...
async function createSkillSnapshot(pkg, changeDir) {
  ...
  const snapshotsRoot = path15.resolve(changeDir, ".comet", "skill-snapshots");
  const snapshotDir = path15.join(snapshotsRoot, material.hash);
  ...
  const temporaryDir = path15.join(snapshotsRoot, `.tmp-${randomUUID2()}`);
  ...
  try {
    ...
    for (const file of material.files) {
      const destination = path15.join(temporaryDir, ...file.path.split("/"));
      assertInside(temporaryDir, destination, `Snapshot file ${file.path}`);
      await fs9.mkdir(path15.dirname(destination), { recursive: true });
      await fs9.writeFile(destination, file.content);
    }
    await fs9.writeFile(path15.join(temporaryDir, "package.json"), packageJson(material.document));
    await fs9.writeFile(path15.join(temporaryDir, "sha256"), material.hash + "\n");
...
async function readSkillSnapshot(changeDir, hash) {
  ...
  const snapshotsRoot = path15.resolve(changeDir, ".comet", "skill-snapshots");
  const snapshotDir = path15.join(snapshotsRoot, hash);
  ...
  try {
    ...
    const packagePath = path15.join(snapshotDir, "package.json");
...
async function migrationArtifacts(changeDir, evidence) {
  ...
  const progress = path16.join(changeDir, "subagent-progress.md");
  if (await pathExists2(progress)) {
    artifacts.subagent_progress = path16.relative(projectRoot, progress).split(path16.sep).join("/");
  }
...
async function ensureClassicRun(changeDir, options) {
  ...
  if (projection.run) {
    ...
    if (installedHash !== projection.run.skillHash) {
      ...
      return {
        ...
        migrated: false,
        snapshotDir: path16.join(changeDir, ".comet", "skill-snapshots", projection.run.skillHash)
      };
    }
  ...
  const expectedSnapshotDir = path16.join(changeDir, ".comet", "skill-snapshots", expectedHash);
  ...
  try {
    ...
    if (handoff?.source) {
      ...
      createdFiles.push(path16.resolve(changeDir, run.contextRef));
    }
    await writeArtifacts(changeDir, run.artifactsRef, artifacts);
    createdFiles.push(path16.resolve(changeDir, run.artifactsRef));
    ...
    createdFiles.push(path16.resolve(changeDir, run.checkpointRef));
    createdFiles.push(path16.resolve(changeDir, run.trajectoryRef));
...
async function isClassicRuntimePackageRoot(root) {
  ...
  if (await fileExists2(path17.join(root, "skill.yaml"))) return true;
  return await fileExists2(path17.join(root, "SKILL.md")) && await fileExists2(path17.join(root, "comet", "skill.yaml"));
}
...
async function classicRuntimeRoot() {
  const runtimeDirectory = path17.dirname(fileURLToPath(import.meta.url));
  const candidates = [
    process.env.COMET_RUNTIME_CLASSIC_ROOT,
    path17.resolve(runtimeDirectory, "..", "runtime", "classic"),
    path17.resolve(runtimeDirectory, "..", "..", "comet", "runtime", "classic"),
    path17.resolve(runtimeDirectory, "..", "..", "assets", "skills", "comet", "runtime", "classic"),
    path17.resolve("assets", "skills", "comet", "runtime", "classic"),
    process.env.COMET_CLASSIC_SKILL_ROOT,
    path17.resolve(runtimeDirectory, "..", "..", "comet-classic"),
    path17.resolve(runtimeDirectory, "..", "..", "assets", "skills", "comet-classic"),
    path17.resolve("assets", "skills", "comet-classic")
...
async function loadClassicRuntimePackage(root) {
  if (await fileExists2(path17.join(root, "skill.yaml"))) {
...
async function ensureClassicRuntimeRun(changeDir) {
  ...
  return ensureClassicRun(changeDir, {
    skillPackage: root ? await loadClassicRuntimePackage(root) : embeddedClassicRuntimePackage(path17.dirname(fileURLToPath(import.meta.url)))
  });
}
...
var CLASSIC_STATE_EVENT_LOG = path18.join(".comet", "state-events.jsonl");
...
async function resolveCommandContext(options) {
  const invocationCwd = path19.resolve(options.invocationCwd ?? process.cwd());
  const projectRoot = path19.resolve(
    options.projectRoot ?? await discoverClassicProject(invocationCwd)
  );
  const [realInvocationCwd, realProjectRoot] = await Promise.all([
    fs12.realpath(invocationCwd),
    fs12.realpath(projectRoot)
  ]);
...
var classicValidateCommand = async (args, options) => withClassicCommandContext(options, async () => {
  ...
  const yamlFile = path20.join(directory, ".comet.yaml");
  ...
  try {
    ...
    if (error.code === "ENOENT") {
      fail2(".comet.yaml does not exist");
      lines.push(
        "",
  ...
  if (document.errors.length > 0 || !(0, import_yaml4.isMap)(document.contents)) {
    for (const error of document.errors) fail2(error.message);
    if (!(0, import_yaml4.isMap)(document.contents)) fail2("document root must be a mapping");
  ...
  for (const field2 of REQUIRED) {
    if (!Object.prototype.hasOwnProperty.call(record, field2)) {
      fail2(`missing required field '${field2}'`);
    }
  }
  for (const [field2, values] of Object.entries(ENUMS)) {
    ...
    if (!value) {
      if (field2 === "auto_transition") {
        fail2(`${field2}='' is not valid. Expected: ${values.join(" ")}`);
      }
      continue;
    }
    if (!values.includes(value)) {
      fail2(`${field2}='${value}' is not valid. Expected: ${values.join(" ")}`);
    }
  }
  if (Object.prototype.hasOwnProperty.call(record, "bound_branch")) {
    const value = record.bound_branch;
    if (value !== null && typeof value !== "string") {
      fail2(`bound_branch='${text(value)}' is not a string or null`);
    }
  }
  if (Object.prototype.hasOwnProperty.call(record, "verify_failures")) {
    const value = record.verify_failures;
    if (typeof value !== "number" || !Number.isInteger(value) || value < 0) {
      fail2(`verify_failures='${text(value)}' is not a non-negative integer`);
    }
  }
  for (const field2 of ["design_doc", "plan", "handoff_context", "verification_report"]) {
    ...
    if (!value) continue;
    if (/^(?:[A-Za-z]:|[\\/]|~)/u.test(value) || value.split(/[\\/]/u).includes("..")) {
      fail2(`${field2}='${value}' must be a relative repository path`);
      continue;
    }
    try {
      if (!await classicProjectTargetExists(projectRoot, path20.resolve(projectRoot, value), {
        label: `${field2} artifact pointer`,
        expected: "file"
      })) {
        fail2(`${field2}='${value}' does not exist on disk`);
      }
    } catch (error) {
      fail2(
  ...
  for (const field2 of ["handoff_hash"]) {
    ...
    if (value && !/^[a-f0-9]{64}$/u.test(value)) {
      fail2(`${field2}='${value}' is not a sha256 hex digest`);
    }
  }
...
async function resolveBranchBinding(changeDir, options) {
  const file = path22.join(changeDir, ".comet.yaml");
...
async function exists(file) {
  ...
  return classicProjectTargetExists(projectRoot, file, {
    label: `Classic guard path ${path23.relative(projectRoot, path23.resolve(projectRoot, file)).replaceAll("\\", "/")}`
  });
}
async function nonempty(file) {
  ...
  return classicProjectFileNonempty(
    projectRoot,
    file,
    `Classic guard file ${path23.relative(projectRoot, path23.resolve(projectRoot, file)).replaceAll("\\", "/")}`
  );
}
...
async function readField(changeDir, field2) {
  const file = path23.join(changeDir, ".comet.yaml");
  const document = (0, import_yaml6.parseDocument)(
...
async function handoffSourceFiles(changeDir) {
  ...
  if (await exists(specs)) {
    ...
    for (const entry of (await fs14.readdir(specs)).sort()) {
...
async function preflight(changeDir, name) {
  ...
  if (!await exists(path23.join(changeDir, ".comet.yaml"))) {
    throw new GuardFailure(red(`FATAL: .comet.yaml not found in ${changeDir}`));
  }
  await inspectClassicProjectTarget(classicCommandProjectRoot(), path23.join(changeDir, ".comet"), {
...
function invocationTarget(relative) {
  return path23.resolve(classicCommandInvocationCwd(), relative);
}
...
async function tasksAllDone(changeDir) {
  const tasks = path23.join(changeDir, "tasks.md");
...
Next: restore or create tasks.md for this change before leaving build.`
  ...
  const unfinished = source.split(/\r?\n/u).map((line, index) => ({ line, number: index + 1 })).filter((entry) => /^- \[ \]/u.test(entry.line));
...
Next: restore the Superpowers plan file or update .comet.yaml plan before leaving build.`
  ...
  const unfinished = source.split(/\r?\n/u).map((line, index) => ({ line, number: index + 1 })).filter((entry) => /^\s*- \[ \]/u.test(entry.line));
  if (unfinished.length > 0) {
    return fail(
...
async function guardOpenChecks(output, changeDir) {
  ...
  const checks = [
    check(
      ...
      async () => await nonempty(path23.join(changeDir, "proposal.md")) ? pass() : fail("")
    ),
    check(
      "proposal.md matches configured language",
      () => documentLanguageMatchesConfigured(changeDir, path23.join(changeDir, "proposal.md"))
    ),
    check(
      "tasks.md exists and non-empty",
      async () => await nonempty(path23.join(changeDir, "tasks.md")) ? pass() : fail("")
    ),
    check(
      "tasks.md matches configured language",
      () => documentLanguageMatchesConfigured(changeDir, path23.join(changeDir, "tasks.md"))
    ),
    check(
  ...
  if (workflow === "full") {
    checks.splice(
      ...
      check(
        ...
        async () => await nonempty(path23.join(changeDir, "design.md")) ? pass() : fail("")
      ),
      check(
        "design.md matches configured language",
        () => documentLanguageMatchesConfigured(changeDir, path23.join(changeDir, "design.md"))
      )
    );
  }
...
async function guardDesignChecks(output, changeDir, change) {
  ...
  if (designDoc && designDoc !== "null") {
    blocked = await runChecks(output, [
      check(
...
async function applyStateUpdate(output, change, changeDir, phase) {
  ...
  const message = phase === "open" ? template.replace("PLACEHOLDER", result.classic.phase) : template;
...
// domains/comet-classic/classic-guard-entry.ts
