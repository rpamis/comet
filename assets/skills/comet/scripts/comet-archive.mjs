...
var require_cst_visit = __commonJS({
  "node_modules/.pnpm/yaml@2.9.0/node_modules/yaml/dist/parse/cst-visit.js"(exports) {
    ...
    visit.itemAtPath = (cst, path21) => {
      let item = cst;
      for (const [field2, index] of path21) {
    ...
    visit.parentCollection = (cst, path21) => {
      ...
      const field2 = path21[path21.length - 1][0];
      ...
      for (const field2 of ["key", "value"]) {
        ...
        if (token && "items" in token) {
          for (let i = 0; i < token.items.length; ++i) {
            const ci = _visit(Object.freeze(path21.concat([[field2, i]])), token.items[i], visitor);
            if (typeof ci === "number")
              i = ci - 1;
            else if (ci === BREAK)
              return BREAK;
...
function requiredRunReference(doc, key) {
  ...
  if (path10.isAbsolute(value) || /^(?:[A-Za-z]:|[\\/]|~)/u.test(value) || value.split(/[\\/]/u).includes("..")) {
...
function classicLayoutPaths(projectRoot, artifactLayout) {
  ...
  const openSpecBase = artifactLayout === "docs" ? path4.join(root, "docs") : root;
  const openSpecRoot = path4.join(openSpecBase, "openspec");
  const superpowersRoot = path4.join(root, "docs", "superpowers");
  return {
    projectRoot: root,
    artifactLayout,
    openSpecBase,
    openSpecRoot,
    changesDir: path4.join(openSpecRoot, "changes"),
    archiveDir: path4.join(openSpecRoot, "changes", "archive"),
    specsDir: path4.join(openSpecRoot, "specs"),
    superpowersRoot,
    superpowersSpecsDir: path4.join(superpowersRoot, "specs"),
    superpowersPlansDir: path4.join(superpowersRoot, "plans"),
    superpowersReportsDir: path4.join(superpowersRoot, "reports")
  };
}
...
async function assertClassicLayoutWritable(projectRoot, artifactLayout, options = {}) {
  const pendingMove = path4.join(path4.resolve(projectRoot), ".comet", "classic-root-move.json");
...
async function discoverClassicProject(startPath) {
  let cursor = path4.resolve(startPath);
  let openSpecFallback = null;
  try {
    if (!(await fs5.lstat(cursor)).isDirectory()) cursor = path4.dirname(cursor);
  } catch (error) {
    if (!isMissingPath2(error)) throw error;
  }
  for (; ; ) {
    if (path4.basename(cursor) === "openspec") {
      openSpecFallback = path4.dirname(cursor);
    }
    const configFile = path4.join(cursor, ".comet", "config.yaml");
    ...
    if (projectConfig || await fileExists(path4.join(cursor, ".git"))) {
      return cursor;
    }
    const parent = path4.dirname(cursor);
    if (parent === cursor) return openSpecFallback ?? path4.resolve(startPath);
    cursor = parent;
  }
}
function classicProjectRelative(projectRoot, target) {
  return path4.relative(path4.resolve(projectRoot), target).replaceAll("\\", "/");
}
...
async function assertClassicOpenSpecRootHealthy(projectRoot, layout) {
  ...
  const configPath = path5.join(resolvedLayout.openSpecRoot, "config.yaml");
...
async function executeClassicOpenSpec(args, startPath = process.cwd()) {
  ...
  if (result.error) {
    ...
    return {
      ...
      stderr: result.stderr || (code === "ENOENT" ? `OpenSpec CLI not found: ${command}` : result.error.message)
    };
  }
  return {
...
function projectTarget(projectRoot, target, label) {
  ...
  const absolute = path6.isAbsolute(target) ? path6.resolve(target) : path6.resolve(root, target);
  const relative = path6.relative(root, absolute).replaceAll("\\", "/");
  return {
    root,
...
async function inspectChangeDirectory(projectRoot, directory, label) {
  ...
  const state = await inspectClassicProjectTarget(
    projectRoot,
    path7.join(directory, ".comet.yaml"),
    {
      label: `${label} state`,
      expected: "file"
    }
...
async function findArchiveChangeDirectory(projectRoot, archiveDir, name, options = {}) {
  ...
  for (const entry of await fs6.readdir(archiveDir, { withFileTypes: true })) {
    ...
    const candidate = await inspectChangeDirectory(
      projectRoot,
      path7.join(archiveDir, entry.name),
...
async function inspectClassicActiveChangeDirectory(name, projectRoot = process.cwd()) {
  ...
  const inspection = await inspectChangeDirectory(
    layout.projectRoot,
    path7.join(layout.changesDir, name),
...
async function findClassicArchiveChangeDirectory(name, projectRoot = process.cwd(), options = {}) {
  ...
  if (options.preferredArchiveName !== void 0) {
    ...
    const inspection = await inspectChangeDirectory(
      layout.projectRoot,
      path7.join(layout.archiveDir, preferred),
...
async function readLegacyArchivedHandoffFallback(projectRoot, changeDir, pointer, maxBytes) {
  ...
  const archivedChange = path8.resolve(changeDir);
  if (path8.dirname(archivedChange) !== path8.resolve(layout.archiveDir)) return null;
  const archivedName = path8.basename(archivedChange);
  ...
  const mapped = path8.join(archivedChange, ".comet", ...suffix);
...
function relativeSource(projectRoot, file) {
  return path9.relative(projectRoot, file).split(path9.sep).join("/");
}
...
async function deltaSpecEvidence(projectRoot, changeDir) {
  const specsDir = path9.join(changeDir, "specs");
  let entries;
  try {
  ...
  const candidates = entries.map((entry) => path9.join(specsDir, entry, "spec.md"));
...
async function collectClassicEvidence(changeDir, projection) {
  ...
  const proposal = path9.join(changeDir, "proposal.md");
  const design = path9.join(changeDir, "design.md");
  const tasks = path9.join(changeDir, "tasks.md");
  const checkpoint = projection.run ? path9.resolve(changeDir, projection.run.checkpointRef) : path9.join(changeDir, ".comet", "checkpoint.json");
...
function cometCurrentSelectionFile(projectRoot) {
  return path19.join(projectRoot, ".comet", "current-change.json");
}
...
async function clearCometCurrentSelection(projectRoot) {
  await fs12.rm(cometCurrentSelectionFile(projectRoot), { force: true });
}
...
function isPathInside(parent, target) {
  const relative = path20.relative(parent, target);
  return relative !== "" && !path20.isAbsolute(relative) && relative !== ".." && !relative.startsWith(`..${path20.sep}`);
}
function archivedPointer(projectRoot, activeDir, archiveDir, pointer) {
  if (!pointer) return pointer;
  const absolute = path20.resolve(projectRoot, pointer);
  if (!isPathInside(activeDir, absolute)) return pointer;
  return classicProjectRelative(
    projectRoot,
    path20.join(archiveDir, path20.relative(activeDir, absolute))
  );
}
...
async function verifyFinalArchiveIntegrity(projectRoot, archiveDir) {
  ...
  for (const [field2, pointer] of [...statePointers, ...artifactPointers]) {
    if (!pointer) continue;
    if (!await classicProjectTargetExists(projectRoot, path20.resolve(projectRoot, pointer), {
...
async function verifyMainSpecsClean(projectRoot, specsRoot) {
  ...
  let found = false;
  const entries = await fs13.readdir(specsRoot, { withFileTypes: true });
...
var classicArchiveCommand = async (args) => {
  ...
  try {
    ...
    let archiveDir = path20.join(layout.archiveDir, archiveName);
    ...
    if (recoveredArchive) {
      ...
      archiveName = path20.basename(recoveredArchive.directory);
    }
    const projection = await readClassicState(changeDir);
    if (!projection.classic) {
    ...
    if (dryRun) {
      ...
      archiveName = path20.basename(resolvedArchive.directory);
...
// domains/comet-classic/classic-archive-entry.ts
