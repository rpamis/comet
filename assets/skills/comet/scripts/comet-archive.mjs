...
var CLASSIC_STATE_EVENT_LOG = path10.join(".comet", "state-events.jsonl");
async function appendClassicStateEvent(changeDir, input) {
  ...
  const file = path10.join(changeDir, CLASSIC_STATE_EVENT_LOG);
  await fs9.mkdir(path10.dirname(file), { recursive: true });
  await fs9.appendFile(file, `${JSON.stringify(record)}
`, "utf8");
  return record;
}
...
async function findArchiveDir(change, preferred) {
  ...
  for (const entry of (await fs10.readdir(archiveRoot)).sort()) {
    if (!entry.endsWith(`-${change}`)) continue;
    const candidate = `${archiveRoot}/${entry}`;
    if ((await fs10.stat(candidate)).isDirectory()) return candidate;
  }
  return null;
}
...
async function annotateFrontmatter(output, file, archiveName, extraFields, dryRun) {
  ...
  const original = await fs10.readFile(file, "utf8");
...
${original}`;
  ...
  await fs10.writeFile(file, updated);
...
var classicArchiveCommand = async (args) => {
  ...
  try {
    ...
    if (recoveredArchive) {
      ...
      archiveName = path11.basename(recoveredArchive);
    }
    const projection = await readClassicState(changeDir);
    if (!projection.classic) {
    ...
    if (dryRun) {
      ...
      archiveName = path11.basename(resolvedArchive);
