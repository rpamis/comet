...
async function listEnabledActiveChanges(projectRoot, enabled, dependencies, cached) {
  const [native, classic] = await Promise.all([
    enabled.includes("native") ? cached?.workflow === "native" ? cached.candidates : dependencies.listNative(projectRoot) : [],
    enabled.includes("classic") ? cached?.workflow === "classic" ? cached.candidates : dependencies.listClassic(projectRoot) : []
  ]);
  return [...native, ...classic];
}
function resolveActiveCandidates(candidates, staleSelection) {
