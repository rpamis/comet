      ...
      for (const field2 of ["key", "value"]) {
        ...
        if (token && "items" in token) {
          for (let i = 0; i < token.items.length; ++i) {
            const ci = _visit(Object.freeze(path8.concat([[field2, i]])), token.items[i], visitor);
            if (typeof ci === "number")
              i = ci - 1;
            else if (ci === BREAK)
              return BREAK;
...
function filesystemPath(relativePath2) {
  return path2.resolve(...relativePath2.split("/"));
}
...
async function resolveClassicChangeDirectory(name) {
  ...
  if (await exists(filesystemPath(archiveRoot))) {
    const matches = [];
    for (const entry of await fs2.readdir(filesystemPath(archiveRoot), { withFileTypes: true })) {
      if (!entry.isDirectory() || !entry.name.endsWith(`-${name}`)) continue;
      const candidate = `${archiveRoot}/${entry.name}`;
      if (await exists(path2.join(filesystemPath(candidate), ".comet.yaml"))) {
...
var CLASSIC_STATE_EVENT_LOG = path6.join(".comet", "state-events.jsonl");
async function appendClassicStateEvent(changeDir, input) {
  ...
  const file = path6.join(changeDir, CLASSIC_STATE_EVENT_LOG);
  await fs6.mkdir(path6.dirname(file), { recursive: true });
  await fs6.appendFile(file, `${JSON.stringify(record)}
`, "utf8");
  return record;
}
...
async function nonempty(file) {
  try {
    return (await fs7.stat(file)).size > 0;
  } catch (error) {
    if (error.code === "ENOENT") return false;
    throw error;
  }
...
async function readDocument2(file) {
  let source;
  try {
    source = await fs7.readFile(file, "utf8");
  } catch (error) {
    if (error.code === "ENOENT") {
      fail(
        `ERROR: .comet.yaml not found at ${path7.relative(process.cwd(), file).replaceAll("\\", "/")}`
      );
    }
    throw error;
  }
  const document = (0, import_yaml3.parseDocument)(source, { uniqueKeys: false });
...
async function atomicWrite(file, content) {
  ...
  try {
    await fs7.writeFile(temporary, content, "utf8");
...
async function projectConfigValue(field2) {
  const file = path7.resolve(".comet", "config.yaml");
...
async function stateFile(name) {
  ...
  return {
    ...change,
    file: path7.join(change.directory, ".comet.yaml")
  };
}
...
function parsedValue(field2, value) {
  const document = (0, import_yaml3.parseDocument)(`${field2}: ${value}
`);
...
async function setField2(output, name, field2, value, options = {}) {
  if (MACHINE_OWNED_FIELDS.has(field2) && !options.machineOwned) {
    fail(`ERROR: '${field2}' is a machine-owned Run field and cannot be set directly`);
  }
...
async function init(output, name, workflow) {
  ...
  if (await exists2(file)) fail(`ERROR: .comet.yaml already exists at ${label}/.comet.yaml`);
...
async function requireOpenArtifacts(name) {
  ...
  for (const artifact of ["proposal.md", "tasks.md"]) {
    if (!await nonempty(path7.join(directory, artifact))) {
      fail(
        `ERROR: Cannot transition '${name}': ${artifact} must exist and be non-empty before leaving open`
      );
    }
  }
  if (workflow === "full" && !await nonempty(path7.join(directory, "design.md"))) {
    fail(
...
async function requireDesignEvidence(name) {
  ...
  if (!designDoc || designDoc === "null" || !await nonempty(path7.resolve(designDoc))) {
    fail(
...
async function writeSparseTransitionEffects(directory, effects) {
  const file = path7.join(directory, ".comet.yaml");
  const document = await readDocument2(file);
...
async function applyTransitionEvent(output, name, event) {
  ...
  if (!classic) {
    ...
    const document = await readDocument2(path7.join(directory, ".comet.yaml"));
...
async function transition(output, name, event) {
  ...
  if (event === "open-complete") {
    ...
    if (!report || !await exists2(path7.resolve(report))) {
      fail(
...
async function next(output, name) {
  ...
  if (!await exists2(file)) fail(`ERROR: .comet.yaml not found at ${label}/.comet.yaml`);
...
async function taskCheckoff(output, taskFile, taskText) {
  ...
  const file = path7.resolve(taskFile);
  if (!await exists2(file)) fail(`ERROR: Task file not found: ${taskFile}`);
  const lines = (await fs7.readFile(file, "utf8")).split(/\r?\n/u);
...
async function check(output, name, phase) {
  ...
  if (phase === "design") {
    ...
    for (const artifact of ["proposal.md", "design.md", "tasks.md"]) {
      (await nonempty(path7.join(directory, artifact)) ? pass : reject)(
        `${artifact} ${await nonempty(path7.join(directory, artifact)) ? "non-empty" : "missing or empty"}`
      );
    }
  } else if (phase === "build") {
    const workflow = await readField(name, "workflow");
    const designDoc = await readField(name, "design_doc");
    if (workflow === "full") {
      (designDoc && designDoc !== "null" && await exists2(path7.resolve(designDoc)) ? pass : reject)(`design_doc=${designDoc} (expected: non-null and file exists)`);
    } else {
...
function fieldStatus(field2, value, file) {
  ...
  if (file && !existsSync(path7.resolve(file))) {
...
async function recoverOpen(output, directory) {
  ...
  let complete = 0;
  for (const artifact of ["proposal.md", "design.md", "tasks.md"]) {
    const done = await nonempty(path7.join(directory, artifact));
    if (done) complete += 1;
...
async function recoverDesign(output, name, directory) {
  ...
  for (const artifact of ["proposal.md", "design.md", "tasks.md"]) {
    output.stdout.push(
      `  - ${artifact}: ${await nonempty(path7.join(directory, artifact)) ? "DONE" : "MISSING (unexpected in design phase)"}`
    );
  }
  ...
  if (design && design !== "null" && await exists2(path7.resolve(design))) {
    output.stdout.push(
      "Recovery action: Design Doc already created and linked. Run guard to transition to build."
    );
  } else if (handoff && handoff !== "null" && await exists2(path7.resolve(handoff))) {
    output.stdout.push(
...
async function recoverBuild(output, name, directory, workflow) {
  ...
  const tasks = path7.join(directory, "tasks.md");
  ...
  const lines = (await fs7.readFile(tasks, "utf8")).split(/\r?\n/u);
  const total = lines.filter((line) => /^\s*- \[[ xX]\] /u.test(line)).length;
  const done = lines.filter((line) => /^\s*- \[[xX]\] /u.test(line)).length;
  const pending = total - done;
  let planTotal = 0;
  let planDone = 0;
  if (plan && plan !== "null" && await exists2(path7.resolve(plan))) {
    const planLines = (await fs7.readFile(path7.resolve(plan), "utf8")).split(/\r?\n/u);
...
async function recoverVerify(output, name) {
  ...
  output.stdout.push(
    ...
    "",
    result === "pass" && branch === "handled" ? "Recovery action: Verification complete. Run guard to transition to archive." : result === "fail" ? "Recovery action: Verification failed and rolled back to build. Resume from /comet-build." : "Recovery action: Verification not yet started or in progress. Run scale assessment then verify."
  );
}
...
async function scale(output, name) {
  ...
  const { file, directory, label } = await stateFile(name);
  if (!await exists2(file)) fail(`ERROR: .comet.yaml not found at ${label}/.comet.yaml`);
  const tasksFile = path7.join(directory, "tasks.md");
  const taskCount = await exists2(tasksFile) ? (await fs7.readFile(tasksFile, "utf8")).split(/\r?\n/u).filter((line) => /^- \[/u.test(line)).length : 0;
  const specs = path7.join(directory, "specs");
  let deltaSpecs = 0;
  if (await exists2(specs)) {
    for (const entry of await fs7.readdir(specs)) {
      if (await exists2(path7.join(specs, entry, "spec.md"))) deltaSpecs += 1;
    }
  }
  const plan = await readField(name, "plan");
  let baseRef = "";
  if (plan && plan !== "null" && await exists2(path7.resolve(plan))) {
    const match = (await fs7.readFile(path7.resolve(plan), "utf8")).match(/^base-ref:\s*(.+)$/mu);
