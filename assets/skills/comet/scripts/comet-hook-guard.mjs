...
var require_cst_visit = __commonJS({
  "node_modules/.pnpm/yaml@2.9.0/node_modules/yaml/dist/parse/cst-visit.js"(exports) {
    ...
    visit.itemAtPath = (cst, path11) => {
      let item = cst;
      for (const [field2, index] of path11) {
    ...
    visit.parentCollection = (cst, path11) => {
      ...
      const field2 = path11[path11.length - 1][0];
      ...
      for (const field2 of ["key", "value"]) {
        ...
        if (token && "items" in token) {
          for (let i = 0; i < token.items.length; ++i) {
            const ci = _visit(Object.freeze(path11.concat([[field2, i]])), token.items[i], visitor);
            if (typeof ci === "number")
              i = ci - 1;
            else if (ci === BREAK)
              return BREAK;
...
import { existsSync, promises as fs9, readFileSync } from "fs";
...
function validateSkillPackage(pkg) {
  ...
  for (const tool of definition.tools) {
    ...
    const normalized2 = path6.posix.normalize(tool.source.replaceAll("\\", "/"));
    if (path6.isAbsolute(tool.source) || normalized2 === ".." || normalized2.startsWith("../")) {
...
async function readPackageFile(root, relativePath2, label) {
  ...
  if (path7.posix.isAbsolute(normalized2) || normalized2 === ".." || normalized2.startsWith("../")) {
    throw new Error(`${label} resolves outside the Skill package`);
  }
  const target = path7.resolve(root, ...normalized2.split("/"));
...
function parseProjectRoot(args) {
  ...
  return path10.resolve(value && !value.startsWith("--") ? value : process.cwd());
}
function relativeToProjectRoot(target, projectRoot) {
  const relative = normalized(path10.relative(projectRoot, target));
  if (relative === "") return "";
  if (relative.startsWith("../") || relative === ".." || path10.isAbsolute(relative)) return null;
  return relative;
}
async function projectRelative(target, projectRoot) {
  const rawCandidate = path10.isAbsolute(target) ? target : path10.resolve(process.cwd(), target);
  ...
  try {
    const parent = await fs9.realpath(path10.dirname(rawCandidate));
    const physicalCandidate = path10.join(parent, path10.basename(rawCandidate));
    const physicalRoot = await fs9.realpath(projectRoot);
    ...
    if (!path10.isAbsolute(target)) return normalized(target).replace(/^\.\//u, "");
  }
...
async function activeChanges(projectRoot) {
  const changesDir = path10.join(projectRoot, "openspec", "changes");
  const governingChanges = [];
  if (!existsSync(changesDir)) return governingChanges;
  for (const entry of (await fs9.readdir(changesDir, { withFileTypes: true })).sort(
    (left, right) => left.name.localeCompare(right.name)
  )) {
    if (!entry.isDirectory() || entry.name === "archive") continue;
    const changeDir = path10.join(changesDir, entry.name);
    if (!existsSync(path10.join(changeDir, ".comet.yaml"))) continue;
...
async function governingChange(relativePath2, projectRoot) {
  ...
  if (relativePath2.startsWith(prefix)) {
    ...
    if (name && name !== "archive") {
      const changeDir = path10.join(projectRoot, "openspec", "changes", name);
      const stateFile = path10.join(changeDir, ".comet.yaml");
