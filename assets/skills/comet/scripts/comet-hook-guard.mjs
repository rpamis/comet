...
var require_cst_visit = __commonJS({
  "node_modules/.pnpm/yaml@2.9.0/node_modules/yaml/dist/parse/cst-visit.js"(exports) {
    ...
    visit.itemAtPath = (cst, path12) => {
      let item = cst;
      for (const [field2, index] of path12) {
    ...
    visit.parentCollection = (cst, path12) => {
      ...
      const field2 = path12[path12.length - 1][0];
      ...
      for (const field2 of ["key", "value"]) {
        ...
        if (token && "items" in token) {
          for (let i = 0; i < token.items.length; ++i) {
            const ci = _visit(Object.freeze(path12.concat([[field2, i]])), token.items[i], visitor);
            if (typeof ci === "number")
              i = ci - 1;
            else if (ci === BREAK)
              return BREAK;
...
import { promises as fs6, readFileSync } from "fs";
...
async function resolveBranchBinding(changeDir, options) {
  const file = path7.join(changeDir, ".comet.yaml");
  const document = (0, import_yaml2.parseDocument)(await fs5.readFile(file, "utf8"), { uniqueKeys: false });
  if (document.errors.length > 0) {
...
function cometCurrentSelectionFile(projectRoot) {
  return path8.join(projectRoot, ".comet", "current-change.json");
}
...
async function readLegacyState(changeDir) {
  const document = await readDocument(path10.join(changeDir, ".comet.yaml"));
...
function parseProjectRoot(args) {
  ...
  return path11.resolve(value && !value.startsWith("--") ? value : process.cwd());
}
function relativeToProjectRoot(target, projectRoot) {
  const relative = normalized(path11.relative(projectRoot, target));
  if (relative === "") return "";
  if (relative.startsWith("../") || relative === ".." || path11.isAbsolute(relative)) return null;
  return relative;
}
async function physicalPathForPossiblyMissingTarget(target) {
  const resolved = path11.resolve(target);
  const root = path11.parse(resolved).root;
  const missingSegments = [];
  let cursor = resolved;
  while (cursor && cursor !== root) {
    try {
      const physicalBase = await fs6.realpath(cursor);
      return path11.join(physicalBase, ...missingSegments.reverse());
    } catch (error) {
      const code = error.code;
      if (code !== "ENOENT" && code !== "ENOTDIR") throw error;
      missingSegments.push(path11.basename(cursor));
      cursor = path11.dirname(cursor);
    }
  }
  try {
    const physicalRoot = await fs6.realpath(root);
    return path11.join(physicalRoot, ...missingSegments.reverse());
  } catch {
    return null;
  }
}
async function projectRelative(target, projectRoot) {
  const rawCandidate = path11.isAbsolute(target) ? target : path11.resolve(process.cwd(), target);
  ...
  try {
    ...
    if (!path11.isAbsolute(target)) return normalized(target).replace(/^\.\//u, "");
  }
...
function governingChangeName(governing) {
  return governing.changeDir ? path11.basename(governing.changeDir) : null;
}
...
// domains/comet-classic/classic-hook-guard-entry.ts
