...
3. After entering the workspace and obtaining `phase`, retrieve context once via [memory integration](reference/commands.md#memory-integration). Expand as needed and record use; at completion, handle Project Memory and Personal Memory separately, then call `comet task --complete`.

Project experience and personal preferences are stored separately. Before completion, write reusable project experience to Project Memory with `comet knowledge remember`; keep Personal Memory for preferences and stable collaboration.
