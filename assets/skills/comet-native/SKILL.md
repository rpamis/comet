...
When status contains `children`, the current change is a Supervisor Change: do not run a Supervisor Change Builder; advance only Runtime's `readyChildren` and Supervisor tasks. Every child is an ordinary Native change and must be created in an independent worktree targeting the Supervisor Change's `workspace.changeBranch`, based on the current integration HEAD. Builder and Verifier results must carry the current `runId`; duplicate or late results are rejected. Children without dependencies may Build and Verify in parallel, but Archive them one at a time with `finish=merge` into the Supervisor Change branch. A Child counts as `done` only after it moves through `active -> verified -> integrated` and its minimum integration checks pass; an Agent completion message or uncommitted files never means `integrated`. After every child is done, execute the Supervisor Change continuation to enter final Verify, where a fresh Verifier checks the complete acceptance list on the final integrated branch. When Runtime returns `parentAdvance`, consume it immediately and notify the user without asking the user to say “advance” again. If Verify fails, preserve conflict/blocker evidence, do not reopen an integrated or archived child, follow `repair-child`, add the actual failed Spec acceptance text to the v2 `acceptance_index`, append a uniquely named repair child, reconfirm Shape, and continue. Keep the target branch unchanged until final delivery; final Archive, workspace finish, merge, push, and PR remain under the existing authorization boundary.
