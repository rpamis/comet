...
- Build: if the Runtime shows `repairing`, Verify has returned to Build. For an ordinary change, keep the current iteration and continue implementation. For a parent change, follow `repair-child` and add an unfinished child covering the failed acceptance items; do not reopen an archived child.
