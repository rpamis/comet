...
As soon as a decision is confirmed, write it into Decisions and the complete target specifications, then remove the corresponding blocker. Runtime persists final Shape confirmation through its `await-user` boundary; do not add a confirmation blocker to the brief. Formal artifacts record conclusions and reasons, not hidden model reasoning. When the user directly supplies a file, attachment, link, or local path as a requirements source, keep the only source coverage map in `brief.md` at `# Scope` under `## Source coverage`; the complete target Spec does not repeat that source table and instead fully expresses all currently active executable semantics.
