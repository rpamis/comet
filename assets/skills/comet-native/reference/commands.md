...
- recovery details;
- the first `acceptancePage` for Build, Verify, or Archive; Build-after-fail also includes the previous `failed` / `missing` states, page-local gap IDs, and bounded failed check IDs.
...
- Repair: after Verify fail, continuation returns `work-phase` without an immediate advance command so the gaps derived by Runtime from the matrix/envelope are handled first. The signature contains the contract hash, failed/missing acceptance IDs, and failed check IDs; code or scope churn does not reset the same gap. The third identical gap returns a manual stop and permits one override using the exact status signature plus a non-empty summary. The contract-wide limit comes from `native.max_verify_failures` (default 5) and hard-stops when reached; only a newly confirmed contract resets the accumulated count.
