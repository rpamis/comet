...
- Archive: only `archive` completes this phase; `next` cannot substitute for it. First run `--dry-run`, then pass the returned `preflightHash` unchanged to `--expect-preflight`. The runtime recomputes it under the mutation lock and runs a final freshness fence after spec operations but before the `archive-change` move. Drift leaves the active change in place for recovery by the same transaction.
