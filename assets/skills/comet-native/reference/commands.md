...
- Return to Build mid-change: when Verify or Archive receives a new implementation request that belongs to the current change, do not write the project file first; run `--return-to-build` to clear stale verification/archive evidence, reread status, and then declare the new Build artifact. This flag cannot be combined with confirmation, artifacts, verification results, partial scope, or repair evidence. Requests outside the current change should use a separate change.
