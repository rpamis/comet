...
- Supervisor task operations use `supervisor-builder-result`, `supervisor-builder-failure`, `supervisor-verifier-result`, `supervisor-reconnect`, `supervisor-cancel`, and `supervisor-integrate`; every operation must carry the current Runtime task package's `runId`, and late, wrong-role, or duplicate results are rejected. Use `comet native next <change> --max-parallel 1` for explicit serial fallback; the default cap is 2.
