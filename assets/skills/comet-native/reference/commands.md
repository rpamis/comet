...
- Shape: advance after the brief and proposed specifications pass; add `--confirmed` only when this turn contains a high-impact decision the user just confirmed.
- Build: recheck the brief and proposed specifications; provide at least one real project artifact or use `--no-code-reason`. Also pass `--confirmed` when this turn includes a high-impact implementation decision the user just confirmed. If complete scope cannot be proven, the first call returns a scope hash and unattributed items without advancing. Retry only after the user accepts the specific risk, with the exact `--allow-partial-scope`, a reason, and `--confirmed`.
