...
After entering the hotfix workspace and reading the current state `phase`, automatically run `comet task <project-root> --task "<original user request>" --phase "<phase>" --json` and inject task-relevant personal memory and project knowledge. Workflow commands may carry `--comet-task`, `--comet-path`, and `--comet-phase`; the CLI strips these markers and displays selected context. Only when the conversation contains a reusable user preference, project convention, or stable collaboration habit does the Agent run `comet memory observe <project-root> --text "<preference or convention>" --workflow <workflow> --change <change-id> --candidate-key <stable-topic-key> --json`; `--text` contains only that preference or convention, never a task summary, implementation progress, command output, or test result, and the command is skipped when there is no candidate. When a compiler, test, or linter fails, fix the code from its diagnostics and rerun. At the end run `comet task <project-root> --task "<original user request>" --complete --workflow <workflow> --change <change-id> --json`; this only completes the checkpoint and sync and never saves the original task as memory. Without a Hook, this Skill performs the same actions directly; `comet memory context` remains a context-only fallback.
