...
After entering the hotfix workspace, automatically run `comet task <project-root> --task "<original user request>" --phase build --json` and inject task-relevant personal memory and project knowledge. Workflow commands may carry `--comet-task`, `--comet-path`, and `--comet-phase`; the CLI strips these markers and displays selected context. After completion, verification, or review run `comet memory observe`; when a compiler, test, or linter fails, fix the code from its diagnostics and rerun. At the end run `comet task <project-root> --task "<original user request>" --complete --workflow <workflow> --change <change-id> --json` to complete the personal-memory checkpoint. Without a Hook, this Skill performs the same actions directly; `comet memory context` remains a context-only fallback.
