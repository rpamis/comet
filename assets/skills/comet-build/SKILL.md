...
**Evidence recording discipline** (follow this order to avoid rerunning work):

1. Run builds and tests directly until they pass; do not interleave any `comet state` writes in between.
2. Once the tree is quiet, run the required `comet check run` invocations back to back (build and verify, one after another in a single shell call), then immediately run `comet guard <change-name> build --apply`.
3. Do not insert `state set`, `record-check`, or other state writes in between — a `phase` change invalidates all check evidence, and a manual `record-check` shadows Runtime evidence.
4. Commit only after guard passes; tick tasks through `task-complete` so plain text edits do not change the tasks.md input scope.
