...
- **Eval HTML reports**: HTML eval outputs now use a paper-style report layout with default Chinese rendering, an English/Chinese toggle, metric and rubric-dimension explanations, a Python-first chart backend, and inline SVG fallback for rubric deltas, quality-cost trade-offs, and task outcomes, so reports remain readable as standalone artifacts without external chart libraries at view time.
