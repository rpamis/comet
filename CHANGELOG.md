...
- **Eval CONTROL baseline semantics**: CONTROL runs for Comet workflow tasks now measure business-task completion without requiring Comet/OpenSpec/Superpowers Skill invocation or test-artifact checks, while Comet-specific rubric dimensions are marked as not applicable. The former completion score and pass@k/pass^k table are split into business and workflow views so no-skill baselines are not penalized for intentionally missing workflow artifacts.
- **Eval HTML reports**: HTML eval outputs now use a paper-style report layout with default Chinese rendering, an English/Chinese toggle, centered metric tables, metric and rubric-dimension explanations, a Python-first chart backend, and inline SVG fallback for rubric deltas, quality-cost trade-offs, and task outcomes, so reports remain readable as standalone artifacts without external chart libraries at view time.
- **Eval diagnostic evidence**: Flagged-run tables now summarize noisy multi-turn loop traces, sample-level failed checks are labeled separately from task outcomes, repeated attribution causes are grouped with counts, and long evidence or report-path columns stay constrained inside the HTML page, so diagnostic sections remain readable without overflowing the report canvas.
- **Eval pass@k reporting**: Eval comparison reports now include `pass@3` and `pass^3` whenever at least three clean runs are available, so k=3 reliability studies surface the requested capability ceiling and consistency floor directly in Markdown and HTML output.
...
- **Eval report robustness**: Eval comparison reports now read UTF-8 report JSON consistently on Windows and tolerate partially missing rule or LLM-judge overlay scores, so report generation no longer fails on localized artifacts or sparse judge output.
- **Comet observability eval task**: Adds the missing validation script for the LangSmith `.env.example` template benchmark, so the bundled Comet workflow eval task now produces normal report artifacts instead of failing before validation.
...
- **Repository architecture lint**: Architecture lint now respects exact nested cache directories listed in `.gitignore`, so local eval caches such as `eval/.cache/` no longer trigger false code-root violations.
...
- **Eval scaffold stability**: Eval artifact references and saved report files now keep stable treatment filenames.
- **Eval artifact snapshots**: Saved eval workspace artifacts now exclude nested Git metadata, so local editors no longer detect generated `eval/local/logs/.../artifacts/` snapshots as extra repositories with noisy file changes.
- **LangSmith eval result columns**: LangSmith eval experiments now keep failed-run outputs, rubric feedback, and Claude Code child traces attached to the experiment run, so failed CONTROL/baseline cases still show checks, rubric scores, token/cost metrics, and nested trajectory details in LangSmith.
