...
## What's Changed [0.4.0-rc.1] - 2026-08-25

### Added

- **Personal Memory and Agent learning**: Comet now maintains an independent User Profile and task-relevant personal context, learns stable preferences and verified collaboration experience from explicit feedback and completed workflow outcomes, and keeps one-off task output out of durable memory.
- **Personal Memory providers and management**: Personal Memory now supports a built-in Local Provider and an optional Remote Provider, with `comet memory` and Dashboard controls for readable records, correction, permanent deletion, rollback, conflict handling, synchronization, and bounded character budgets.
...
- **Dashboard memory and knowledge workflow**: Personal Memory and Project Knowledge now use readable registries with explicit application history, stable empty and loading states, dedicated settings, aligned desktop three-pane proportions, and clear separation between user memory and project evidence. Personal Memory also explains the latest task-used context in plain language, lets users open the matching memory details, and removes permanently deleted memories from that preview. Built-in memory and knowledge groups now use clearer user-facing names with compact hover and click guidance, origin badges, category-aware empty states, and Topic fields for free-form labels; detail previews now make the selected record name the visual focus without redundant headings, while Project Overview and Project Standards remain visibly distinct views. Project Knowledge query controls now stay with the input in a compact action row, and inspector actions form a compact group with a restrained destructive replacement action.
- **Context preview readability**: Personal Memory detail metadata now uses flat separator lists, while Project Knowledge source previews use readable metadata and code text sizing without nested card styling.
- **Dashboard registry columns**: Personal Memory table columns now share a fixed action track, while Project Knowledge timestamps retain enough width to show complete values.
- **Context preview titles**: Personal Memory and Project Knowledge detail dialogs now use restrained titles without the decorative accent bar.
- **Markdown source previews**: Demo project sources now render their Markdown headings and paragraphs as document content instead of wrapping the entire preview in a code block.
- **Plugin workspace spacing**: Personal Memory and Project Knowledge now use a compact, balanced desktop gutter shared by the Header and three-pane workspace instead of leaving a large empty rail beside the navigation.
- **Plugin center header inset**: Personal Memory and Project Knowledge now keep the same horizontal project selector inset as the Classic and Native workflow headers.
- **Workflow header alignment**: Classic and Native workspaces now share the same horizontal gutter between their Header controls and main content, keeping the sidebar junction on a single vertical grid.
- **Plugin rail alignment**: Personal Memory and Project Knowledge now align filter search, selection, and status content with thin separators inset equally from both rail edges.
...
- **Dashboard loading and collapsed navigation**: Dashboard loading surfaces now use one consistent line-skeleton treatment across Classic, Native, plugin pages, and source previews, while the collapsed desktop sidebar keeps a visible, clickable Settings icon.
...
- **Project Knowledge context preview**: The Project Knowledge page summarizes the latest task-used context before opening its applied facts or standards in a fullscreen-capable detail card, keeping the workspace focused while preserving application evidence on demand.
- **Dashboard modal controls**: Content dialogs now place one consistent fullscreen/restore control at the far-right edge of the title bar instead of a separate close button; clicking the backdrop or pressing Escape closes normal dialogs and restores fullscreen dialogs without losing their current content.
...
- **Dashboard Personal Memory notices**: Opening or refreshing the Personal Memory page no longer reports its management read as a task memory application, while real task retrieval notices remain available.
- **Project Knowledge view hierarchy**: Data Sources and Query Test now use compact toolbar and inline guidance instead of standalone explanatory headers, keeping the workspace focused on its controls.
