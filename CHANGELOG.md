...
- **LangSmith eval configuration**: LangSmith eval runs now derive Claude Code tracing plugin settings from the primary `LANGSMITH_*` configuration, so users no longer need to configure both `LANGSMITH_TRACING` and the similar-looking `TRACE_TO_LANGSMITH` in normal setups.
