...
- **Memory and knowledge workspaces**: Personal Memory now uses a searchable record registry with persistent application-reason details, while Project Knowledge uses a three-pane category, record, and persistent-inspector registry with dedicated source and retrieval views; the coordinated sidebar can collapse completely when more workspace is needed, all management controls are localized, Native and Classic workspace content remains unchanged, and Dashboard now provides one in-context blurred Settings center that can expand to the full viewport for Personal Memory, Project Rules, and structured `.comet/config.yaml` workflow configuration without discarding the active workspace.
