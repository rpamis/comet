...
- **Cross-device Native lock recovery**: `comet native doctor <change> --repair` takes over change locks whose owner cannot be proven stale (left by a crash on another machine, or an owner whose liveness probe never succeeds) once the owner record is over an hour old, ending the state where every mutating command failed after a five-second lock wait until someone removed the lock by hand. Younger unknown locks stay untouched with the remaining wait reported.
...
- **Leaner phase entries in Skills**: comet-design, comet-verify, and comet-archive skip the entry `state select` + `state check` pair when the previous phase's guard already returned the phase state (matching the existing Build exemption); Open-phase artifact completion validates locally through `state artifacts` and only refreshes OpenSpec status when it reports a problem; known-name Native entry calls `select` directly because it returns the same discovery status would; read-only commands are batched into one shell invocation where the Skills list them; the archive finish steps run as one chained invocation. All Skill changes are published in Chinese and English together.
