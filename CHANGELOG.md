...
- **Classic differential input binding**: an undeclared whole-tree scope reuses its recorded input manifest differentially when one exists — a single `git status` classifies which paths could have changed, untouched entries reuse their recorded digests, and only changed or added paths pay the per-file work — so the second check run and every guard revalidation stop re-stating the entire tree. The digest no longer includes the file mode; chmod-only changes no longer invalidate evidence, and policies that care keep binding modes through `git: all` index entries.
