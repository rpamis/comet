...
## What's Changed [0.4.2] - 2026-09-19
...
- **Classic evidence invalidation details**: When guard reports check evidence as not reusable, it now prints the reason, the changed input files, and the declared relevance scope instead of a generic missing-evidence message; environment-binding failures explain which components are bound (executable path and file identity, Node version, and declared policy variables).
- **Native snapshot selection budget**: Configure `native.snapshot.max_selection_records` in `.comet/config.yaml` to raise the Git selection record and physical enumeration node ceilings (default 20,000), so large monorepos are no longer hard-blocked at change creation and Build evidence by the fixed budget, and the incomplete-baseline error now points at the effective knobs.
- **Native handoff checks**: Submit the final Runtime check plan through Builder handoff `verification_checks`, so Runtime freezes the candidate, returns failed checks to Build, and reuses an unchanged successful plan during Verifier dispatch instead of executing the same complete checks twice. Results identify whether Runtime executed or reused the plan (#429).
- **Classic monorepo build detection**: When the invocation root's package.json has no build script, guard now probes workspace packages (one level of directories declared through `workspaces` or `pnpm-workspace.yaml`). Exactly one package with a build script auto-runs as `npm --prefix <package> run build`; several candidates are listed with a copyable record command instead of a generic missing-evidence message (#432).
- **Native Verifier startup receipts**: The Verifier dispatch package now instructs the Verifier to submit a `verifier-started` runner input as its first Runtime action (binding `candidateId` and `verifierExecutionRef`, repeatable without side effects). `comet native status` reports `localExecution.verifierStartup` — with `unconfirmed`/`confirmed`, registration time, and confirmation time — the human summary appends a "has not confirmed startup" note while an attempt stays unconfirmed, and recovery names attempts that registered but never started, so a written dispatch record is distinguishable from a Verifier that actually launched.
- **ZCode Hook support**: `comet init` and `comet update` now install the Comet phase-guard Hook for ZCode. The Router is registered in `.zcode/config.json` as a project-scoped `PreToolUse` process Hook (`hooks.events.PreToolUse`, `Write|Edit` matcher, `node` argument vector) and the install forces `hooks.enabled: true`, which ZCode requires before configuration-file hooks run; user hooks, sibling events, and unrelated config keys are preserved, `comet uninstall` / `comet update` remove only the managed entries, and `comet doctor` inspects the format (including reporting an installed-but-disabled runner as not present). Hooks stay project-scoped like Claude/Codex, and `~/.zcode/cli/config.json` is targeted for historical global cleanup.
- **Project memory commands**: Record durable project-scoped memories with `comet knowledge remember` as a file-based `MEMORY.md` index plus one Markdown per lesson; the index is injected in full with every task context, single entries expand via `--expand-context "project-memory:<slug>"`, and `comet knowledge forget --memory <slug>` removes stale ones.

### Changed

- **Classic preset scope authorization**: Hotfix and tweak can reuse an explicit per-change ruling when only the delivery-file-count prompt is exceeded, while substantive scope or risk changes still pause for a user decision.
- **Workflow checks and Hook responsiveness**: Classic check policies can declare `outputs` that are excluded from command inputs, and the same reusable full command now carries across Build and Verify without executing twice. Native status, show, and next responses restore persisted check plans after resume. Write Hooks validate only the selected Classic or Native change, reject artifacts owned by another change, and no longer run optional plugin retrieval synchronously, so an unrelated damaged change or slow context provider cannot stall ordinary edits.
- **Select returns the full status projection**: `comet native select <change> --json` now returns the same discovered status body as `status <change>` (workspace, phase, state version, local execution) plus `selected`, and routes the execution context into the owning workspace — completing the Skill promise that a known-name entry needs no separate status call.
- **Cross-device Native lock recovery**: `comet native doctor <change> --repair` takes over change locks whose owner cannot be proven stale (left by a crash on another machine, or an owner whose liveness probe never succeeds) once the owner record is over fifteen minutes old, ending the state where every mutating command failed after a five-second lock wait until someone removed the lock by hand. Younger unknown locks stay untouched with the remaining wait reported.
- **Windows process identity probing**: creation-time identity tries WMIC first (tens of milliseconds, no PowerShell cold start) and falls back to PowerShell where WMIC is absent; a probe failure now resolves to a distinct `unknown` liveness instead of counting as alive forever, so a zombie Runtime check owner becomes takeable through the doctor repair path.
- **Classic empty-lock crash window**: a crash between creating the state lock and writing its owner no longer makes every state, guard, and check command wait out the 30-second lock timeout for five minutes; an empty lock clears after a ten-second grace that covers an in-flight owner write.
- **Hook stdin timeout**: a host that spawns the hook without writing or closing its stdin gets a diagnostic non-zero exit within ten seconds instead of a permanently blocked hook process on every tool call.
- **Subprocess timeouts**: the OpenSpec adapter spawn and the remaining direct Git spawns (state init base-ref, scale diff, branch and worktree probes) carry timeouts, so a wedged subprocess can no longer hang the whole comet command.
- **Native runtime-data recovery guidance**: status no longer points migration-required changes at `doctor --repair` (doctor skips schema migrations) — it names the first mutating `next` command that performs the deterministic migration. Status entries blocked by a broken copy in another worktree now name that workspace so the repair runs in the right directory, and a discarded stale local-execution overlay reports the recorded-versus-current state-version gap that ruled the rebuild.
- **Native check input fingerprint**: the working-tree binding hashes the changed-path set with per-file digests instead of digesting a full `diff --binary` stream, staged content binds through index blob ids, and only execution-relevant environment variables (PATH, NODE_*, locale, temp and home roots, proxy variables) bind instead of every environment entry — incidental environment churn (shell session ids, editor state) no longer breaks check reuse. Recorded fingerprints revalidate after one re-run.
- **Classic differential input binding**: undeclared whole-tree checks use Git object identities for tracked content, hashing only dirty and untracked paths while still detecting content committed after a check. The second check run and guard revalidation avoid rereading every clean file without trusting `git status` alone. The digest no longer includes the file mode; chmod-only changes no longer invalidate evidence, and policies that care keep binding modes through `git: all` index entries.
- **Guard and check duplicate probing**: a command-check evaluation reads the check policy once per evaluation, and executable path resolution is cached per process, so `guard --apply` and `comet check run` stop re-walking PATH for every fingerprint comparison.
- **State select fast path**: re-selecting the change already recorded for the current workspace skips the per-worktree Git enumeration and rebuild checks.
- **One-step isolated archive**: `comet native archive <change> --confirmed --finish <mode>` records the finish choice and archives in one command; the in-transaction freshness recheck replaces the mandatory second full dry-run, and the Skill documents the one-step path.
- **Recovery output without re-reading**: state set/init/transition/select, guard `--apply`, and the check command reuse the projection they just read when assembling recovery context instead of re-reading the change state and run record.
- **Leaner phase entries in Skills**: comet-design, comet-verify, and comet-archive skip the entry `state select` + `state check` pair when the previous phase's guard already returned the phase state (matching the existing Build exemption); Open-phase artifact completion validates locally through `state artifacts` and only refreshes OpenSpec status when it reports a problem; known-name Native entry calls `select` directly because it returns the same discovery status would; the Runner-input pre-check (`--validate-only`) is reserved for diagnosing rejected submissions instead of a routine extra pass; read-only commands are batched into one shell invocation where the Skills list them; the archive finish steps run as one chained invocation. All Skill changes are published in Chinese and English together.

- **Native Verifier dispatch confirmation**: The comet-native Skill no longer treats "dispatched" as "running": the dispatch record only registers the attempt, the platform tool must accept the launch (a rejected or failed launch call is handled as `verifier-execution-error` immediately), and while waiting, `status` distinguishes a registered-but-unstarted attempt from a confirmed startup before the agent keeps waiting or reports a failure.
- **Classic handoff verdict**: `comet handoff <change> --hash-only` now prints a semantic verdict on stderr — `FRESH`, or `STALE` with the changed artifact list and the exact next command — while stdout keeps the bare hash for scripted callers. The comet-verify Skill reads the verdict instead of comparing hashes and querying `handoff_hash` by hand, and reads only the listed changed artifacts when stale.
- **Classic evidence log stat shortcut**: Revalidating recorded check evidence trusts an unchanged log size and mtime instead of rereading the whole log (up to 8 MB per scope) on every guard evaluation; records without the stat and logs whose stat changed keep the full content-hash check, so tampering is still detected.
- **Classic evidence recording guidance**: The comet-build Skill replaces the four-step recording discipline with a single record-and-advance sequence plus error-driven recovery, because Runtime now reuses evidence automatically and guard failures name the exact reason and changed files.
- **Classic decision-point clarity**: The Ambient Resume protocol now defines the question and options for every `ask_user` reason (change selection, dirty-file attribution, pending decision point, missing Comet state, unrelated request), the Design confirmation in comet-design gains an explicit three-option question (approve, request changes, defer), and the comet-build medium-scope-change rule names its two choices instead of a bare “present choices”, so every Classic pause point presents real, selectable options and no stop depends on an open-ended question.
- **Native stale continuation guidance**: A stale `--expected-state-version` rejection now states the current phase, next action, and state version, and includes the `comet native status` recovery command with the arguments to retry, instead of a bare version mismatch, so a moved-on change no longer costs a guessing round-trip.
- **Classic evidence survives documentation edits**: Reuse of recorded build and verify checks no longer breaks when only documentation files changed. Markdown and LICENSE-style files at the repository root or under `docs/`, `doc/`, `documentation/`, and `.github/` are neutral by default (root `.txt`/`.rst` stay bound because they often feed commands; OpenSpec and Superpowers artifacts always stay bound), so editing a README or a docs page no longer forces rerunning the full build and test suite before a guard can advance. Guard and `comet check run` report the ignored document paths instead of silently dropping them, commands with a declared `.comet/check-policy.json` files list keep their exact binding, and `classic.document_evidence: strict` in `.comet/config.yaml` restores whole-tree binding.
- **Classic Hook allows documentation writes in every phase**: The Hook whitelist grows from root Markdown only to the same neutral document set, so finishing a docs page during Open, Design, Verify, or Archive no longer blocks the write or forces a `verify-fail` round-trip that invalidates all check evidence.
- **Native keeps verified candidates across documentation writes**: Ordinary documentation edits during Shape, Verify, and Archive no longer return the change to Build and invalidate the current candidate — they stay outside the implementation scope, matching how Runtime already treats them. Implementation writes still revert the candidate as before, and `native.document_writes: revert` in `.comet/config.yaml` restores the strict behavior.
...
- **Native selection fence respects excludes**: The `git-selection-changed` stability fence no longer watches paths excluded through `native.snapshot.exclude`, so background churn in excluded directories (dev-server logs, generated output) no longer fails Build evidence with a "stabilize the Git index" message that stabilization cannot fix.
...
- **Native source coverage boundaries**: Limit source-document full coverage to the user-selected sections, items, or features plus dependencies needed to understand, implement, or accept them. Unreadable out-of-scope content no longer blocks Shape, while uncertain dependencies that could affect the result still require clarification (#428).
- **Classic check snapshots skip unchanged files**: `comet check run` now reuses the previous execution's input manifest, and the post-execution snapshot reuses the pre-execution one, so a rerun reads only files that actually changed instead of hashing the whole tree twice; guard revalidation keeps reusing recorded manifests. Recorded evidence stays valid across the upgrade and invalidation semantics are unchanged (#432).
- **`comet task` fast startup**: The `task` command now takes the package fast path like the Classic commands, skipping the full Commander entry; its startup time roughly halves, and any argument shape the fast facade does not recognize still falls back to the full CLI with identical diagnostics (#432).
- **Check and archive content size limits removed**: Classic check snapshots and Native archive, checkpoint, root-move, and transaction readers no longer reject large files — a tracked file above the previous 64 MiB cap made every `comet check run` fail with "Classic check input exceeds 67108864 bytes" (#436). Content hashing now streams files in fixed chunks instead of loading them whole, so memory stays bounded regardless of file size, and runtime-internal documents (state, journals, receipts, manifests, progress records) lost their fixed byte caps while keeping structural validation. Structural budgets (entry and file counts, directory depth, durations) are unchanged.
- **Native snapshot and generated-input budgets raised**: The Native baseline snapshot budget (`native.snapshot.max_total_bytes`) and the generated-directory check-input budget default to 1 GB instead of 256 MB/64 MB, so ordinary repositories with large assets no longer produce incomplete baselines or lose check reuse; both still degrade gracefully when exceeded and remain configurable.
- **Native scoped checks skip oversized files**: Files above the scoped checker's per-file limit are now skipped and counted like binary files instead of failing the check receipt, so one large generated asset no longer blocks verification.

### Fixed

- **Paused personal memory learning**: Automatic observations no longer run memory review, persist plaintext, or consume a deduplication key while project learning is paused; after resuming, the same observation can be learned normally, including when upgrading state written by 0.4.1 (#440).
- **Ambient Resume instruction deduplication**: Projects whose root `CLAUDE.md` directly imports `AGENTS.md` no longer receive a duplicate managed recovery block; update migrates existing duplicates, restores the Claude block when the import is removed, and keeps lightweight ownership checks separate from continuation/status recovery (#433).
- **Workflow freshness and convergence**: Classic no longer replaces an explicit failed or interrupted check with an inferred successful build, accepts stale evidence after different content is committed, loses an archived change selection before authorized delivery finishes, clears a newer active selection when another change finishes delivery, or emits shell-unsafe recovery text. `comet check rerun` now repeats the exact Runtime-recorded argv, cwd, timeout, and reuse tier, including when input snapshotting or launch was interrupted before a result existed; a check that modifies undeclared inputs reports the concrete paths and points generated artifacts to the command's `outputs` policy instead of suggesting an endless generic rerun. Native invalidates checks when an already-dirty tracked file or ignored generated input changes, disables fast reuse when a dirty submodule cannot be content-hashed, serializes failure-budget blockers correctly, stops retrying an interrupted check after its third attempt while preserving already-passed local evidence, resets the failure budget for a genuinely revised goal while preserving it for drift recovery, ignores newline or trailing-whitespace-only Shape edits, and recognizes a registered foreign change through a targeted Hook lookup instead of incorrectly telling the user to recreate it. Runtime bundles use the packaged version without reading `package.json`; installed-only Hook Routers expose bounded plugin-context initialization diagnostics without blocking the host; all Runtime builders retry transient Windows generated-file sharing conflicts; and Native plus plugin-store locks bind the process creation identity so PID reuse cannot keep a stale lock active, while legacy owners without an identity remain `unknown` and use controlled repair instead of being mistaken for active work. Chinese and English Verify guidance now runs the first real check through Runtime and uses exact rerun after failure, removing the previous bare-success-then-record duplicate execution.
- **Recoverable archive and delivery fences**: Native clears pre-Archive workspace-finish journals when freshness or ordering checks return a change to recovery, and Doctor can isolate malformed journal entries without following non-file paths. Classic cold recovery reuses an existing invalidation fence instead of growing the trajectory on every restart, keeps archived changes selectable while delivery is pending, and supports explicit reauthorization when a post-Archive branch or remote must change. Optional Hook context enrichment is now best-effort and bounded, so plugin locks and learning replay cannot hold a write Hook indefinitely.
- **State-machine convergence and blocking fixes**: A third audit covering action convergence, concurrency, and waiting boundaries produced these fixes. Builder-handoff Runtime check failures now count against the configured failed-iteration budget instead of looping forever — when the budget is exhausted the candidate stays in Verify with an await-user blocker naming the failed checks. A Verifier dispatch that never confirms startup shows its waiting age in status, and `doctor --repair` force-registers the failure after 30 minutes so a platform queue loss cannot park a change forever. Rerunning a byte-identical Supervisor integration after its checks failed now fails fast with the actual exits instead of returning stale success. Doctor keeps returning diagnostics and a recovery continuation when a pre-upgrade Supervisor overlay has lost `children.yaml`. The failed-iteration budget survives a drift return to Shape (repair rounds can no longer reset it silently; only an explicit `--revise-requirements` clears it). Atomic writes and lock releases retry Windows sharing violations (EPERM/EACCES/EBUSY) with a short backoff, and a lock-release failure after a committed mutation degrades to a warning instead of replacing the success result. Same-host stale root-move locks are taken over automatically on acquisition. Process identity probes are cached per pid, keeping lock paths off repeated PowerShell cold starts. Classic `verify-fail` past three rounds requires `COMET_ACK_VERIFY_FAILURES=1`, the budget-exhausted and retry-verifier messages state their one-shot semantics, and a failed audit-log append after a committed transition degrades to a warning instead of flipping the command to failure.
- **Skill-implementation alignment audit**: Twenty-five documented behaviors that diverged from runtime were aligned across Classic and Native Skills in both languages. Highlights: hotfix and tweak no longer instruct initializing state before choosing worktree isolation (the change directory would not exist in the worktree, so the documented commands failed); the build-phase evidence-recording order no longer expires externally recorded verify evidence at the phase transition; the neutral-document set now matches the runtime whitelist (root Markdown plus LICENSE/NOTICE/AUTHORS; .txt/.rst only under documentation directories); the Supervisor repair flow names the `next --summary` command that returns the change to Shape; lost-Verifier recovery text matches the actual await-user behavior; `needs-reverify` is documented in the child lifecycle; `nextAction` kinds and the `NEXT: delivery` value are documented; memory-observe required options are spelled out; preset escalation notes that workspace bindings including `bound_branch` are cleared; select/status wording matches actual projections; and integration-conflict recovery is documented.
- **Supervisor child state after Shape revisions**: A confirmed children-contract change no longer marks every non-integrated Supervisor child `ready` without checking its dependencies — the re-derivation now applies the same dependency gate the dispatcher asserts, so a child waiting on new or unintegrated dependencies stays `pending` instead of self-locking `advance-children` with "dependency is not integrated" (issue #439). Doctor reports a ready child with unmet dependencies instead of a healthy result, and `doctor --repair` demotes only those stuck children to `pending`, unlocking changes locked by pre-0.4.2 overlays without touching any integrated work.
- **Classic interrupted check diagnostics**: An interrupted check attempt (killed process, tool timeout, or failed launch) previously buried earlier valid evidence behind a generic "No current Runtime build evidence" message; guard now identifies the unfinished attempt and returns the exact `comet check rerun` recovery action (#425).
- **Classic manual record-check shadowing diagnostics**: A manual `state record-check` declaration never satisfied the guard, and it also hid the earlier valid runtime record behind a generic missing-evidence message; guard now names the manual declaration, explains that it shadows runtime evidence until a fresh `comet check run`, and prints the recovery command. The comet-build and comet-verify skills document the evidence recording order — run tests first, record checks back to back on a quiet tree, guard immediately, and keep state writes and commits out of the recording window (#432).
- **Classic symlink check inputs**: A committed symbolic link no longer fails check snapshots with only a bare error; the message now points at the `.comet/check-policy.json` v2 `files` declaration that excludes it.
- **Classic tasks.md resilience**: One malformed or duplicate task line no longer takes down evidence collection, check snapshots, and recovery context; the offending check reports the exact line instead.
- **Classic guard auto-build transparency**: When guard preview auto-runs a detected build command, success, failure, and self-changing input messages now name the executed command and directory and mention the `COMET_SKIP_BUILD=1` opt-out, instead of hiding the invalidation reason behind a silent full build.
- **Classic state diagnostics**: `state checkpoint`/`delivery` field errors list the allowed fields, invalid check policies report the exact offending entry, a missing Classic projection lists the required keys, `classic validate` warns that guard treats unknown `.comet.yaml` fields as fatal, and the guard error names the removal path — the two commands no longer give contradictory verdicts.
- **Native evidence retreat diagnostics**: Retreating non-fresh Verify or Archive evidence back to Build now returns the freshness verdict and what was discarded in its findings instead of an empty list; a partial-allowance hash mismatch reports both the confirmed and current scope hashes; a runtime rebuild summary explains that Verify evidence was reset and the baseline recaptured; the declared-artifact budget and physical-enumeration errors report their counts, limits, and real remedies.
...
- **Native Supervisor Git binding**: Bind `--isolation current` Supervisor changes to the current Git branch at the Shape confirmation boundary when the project initialized Git only after the change was created, instead of dead-ending on "Native parent changes require a Git integration branch". Late binding rejects a dirty baseline or active child tasks, and `comet native doctor` reports the missing binding with the recovery command instead of an unqualified healthy result.
...
- **Runtime internal log byte caps**: The Classic state event log no longer stops at 8 MiB — once reached, every later `state`, `guard`, and `archive` transition failed permanently with no self-healing path — and Native transition and schema-migration journals drop their 512 KiB read cap together with the evidence-retention scan cap on check receipts, so runtime-written documents are bounded by their structural budgets (entry, file, and change counts) instead of a second byte budget (#436).
- **Workspace configuration size caps**: Copying an OpenSpec workspace configuration larger than 1 MiB into a worktree and healing a branch binding on a `.comet.yaml` larger than 2 MiB no longer fail; both reads now follow the same uncapped path the state parser already uses.
- **Large-repository check input index**: Classic check snapshots raise the Git listing buffer to 256 MiB, so `git: all` checks keep working on repositories with hundreds of thousands of tracked files instead of failing with "Cannot inspect check input index".
