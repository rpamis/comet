...
- **Local eval task images**: Installs Claude Code in every task image used by the local Claude runner so quick and full evaluations no longer fail before execution when collecting the benchmark image identity ([#229](https://github.com/rpamis/comet/issues/229)).
