...
- **Memory and knowledge workspaces**: Personal Memory now uses a compact record-focused workspace, while Project Knowledge uses a three-pane category, record, and persistent-inspector registry with dedicated source and retrieval views; the sidebar and plugin surfaces use a readable desktop type scale, all management controls are localized, Native and Classic workspace content remains unchanged, and plugin configuration stays centralized under Settings.
