...
## What's Changed [0.4.0-rc.1] - 2026-08-21
...
- **Agent learning loop**: Comet now journals stable Agent experience events and consolidates reusable Personal Memory and Project Knowledge from explicit preferences, corrections, verification, review decisions, resolved failures, and completed changes. Context selection applies project authority and task selectors, delivers a bounded Context Manifest with on-demand expansion, avoids duplicate delivery within a session, and learns from reported application outcomes.
...
- **Native verification loop**: Native status and transition output now stay compact with fixed-size detail pages, Builder candidates include a separate read-only code review before Verify, repair rounds verify only unresolved or affected scenarios, and Archive still requires one final full verification. Acceptance items now come from brief examples and explicit Spec scenarios instead of descriptive prose fragments.
...
- **Dashboard context and knowledge labels**: Dashboard now explains the latest task-used memory in user-facing language, makes Project Model and Project Policy tabs visibly distinct, identifies built-in knowledge categories, and labels Personal Memory budgets as character limits rather than storage capacity.
- **Memory and knowledge workspaces**: Personal Memory now uses a searchable record registry with the real selection reason, delivery level, and complete application history, while Project Knowledge uses a three-pane category, record, and persistent-inspector registry with the same application evidence plus dedicated source and retrieval views. Both surfaces now share the Dashboard canvas through neutral navigation states, fine dividers, title-free context bars, and consistent controls instead of nested-card styling or decorative gradient icons, reserve their sidebar entries on first paint, and prefetch plugin settings before navigation becomes available. The coordinated sidebar collapses into a persistent icon rail when more workspace is needed, all management controls are localized, Native and Classic workspace content remains unchanged, and Dashboard provides one compact, in-context blurred Settings center that can expand to the full viewport for Personal Memory, Project Rules, and structured `.comet/config.yaml` workflow configuration without discarding the active workspace or showing a second loading screen when opened.
...
- **Project Knowledge retrieval reliability**: Local retrieval now refreshes source-backed records when files change, marks stale records for review instead of injecting them, preserves user corrections over automatic learning, and keeps deterministic learning independent from optional semantic review.
