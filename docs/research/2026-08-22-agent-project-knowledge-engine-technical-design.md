...
| 能力                   | 当前发现                                                                          | 落地决定                                                        |
| ---------------------- | --------------------------------------------------------------------------------- | --------------------------------------------------------------- |
| Local Provider         | `local-provider.ts` 对 Comet 管理的 Markdown 运行有界 ripgrep，再读取命中文件片段 | 保留 ripgrep 强匹配和回退，加入 section SQLite 读模型与混合重排 |
...
| SQLite                 | 当前 Node 22 可用 `node:sqlite` 和 FTS5，但不是所有环境保证                       | 运行时探测；不可用时回退 ripgrep，不要求用户配置                |
...
SQLite 是按 worktree 隔离的用户缓存读模型，与 Dashboard 和 Personal Memory 分离。Markdown section 使用 SQLite FTS5 做候选；项目知识单元保存在受保护的项目维护目录或用户缓存的独立单元仓库，在召回阶段做有界融合。这样可以让单元保持结构化字段和显式共享边界，不为了首期能力复制一套通用图数据库。

### 查询与召回

查询规划提取 strong、phrase、weak 三组词。FTS terms 通道覆盖标题、标题路径、来源、英文标识符和中文术语；trigram 通道补充三字以上中文与 Unicode 子串；ripgrep 用于强匹配、变化来源补充和 SQLite 失效回退。候选使用确定性融合，项目维护单元高于自动单元，当前 Spec/代码高于 Archive，关系只对已经命中的单元做一跳扩展。
...
自动生成的确定性事实在来源核对通过后可直接作为低优先级 active；依赖语义评审的单元必须有成功验证结果才能 active。任何 unit 的来源变化、删除或不支持结论都会阻止召回。关系类型受控，只保留带来源的一跳关系。
...
当前实现已完成 50 条检索基线：混合召回 recall@4 约 0.644，nDCG@4 约 0.862，禁用来源为 0，热路径 p95 约 77ms，精确子集未回退。该结果证明方向可行，但不替代真实 Agent A/B 和全量发布验证。
