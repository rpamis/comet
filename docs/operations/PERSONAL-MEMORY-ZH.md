...
明确的“记住”“以后都这样”或纠正会立即写为 `proven`，下一任务即可生效。只针对“这次”或“当前任务”的要求不会持久化。系统从真实反馈中推断出的可复用经验先进入低优先级 `trial`；来自两个独立成功 change 的证据齐备后才会晋升为 `proven`，被否定、纠正或导致失败时会被改写或标记为 `superseded`。遗忘使用独立 tombstone，旧事件重放不会恢复已遗忘内容。
