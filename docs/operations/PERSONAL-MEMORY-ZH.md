...
- 记忆内容以可读 Markdown 保存：全局偏好在 `profile.md`，项目经验在 `projects/<project-key>.md`。你可以直接查看和编辑这些文件。
...
- secret、凭据、PII、提示注入，以及要求修改 Skill、Agent 指令或项目规则的内容。
...
个人记忆只帮助 Comet 更好地理解你的长期偏好和可复用经验；它不会自动变成团队 Project Rules，也不会修改 Skill、Agent 指令、代码、测试或构建配置。你始终可以查看、纠正、遗忘、回滚、暂停或同步自己的记忆。
