...
每项任务确认完成后、向用户发送最终回复前，使用 Agent Mail CLI 当前已授权的主邮箱 `benyum@agent.qq.com` 向 `675260709@qq.com` 发送完成通知。用户已为这个固定场景授予发送权限，可对 `agently-cli message +send` 使用 `--confirmed`。邮件主题使用 `任务完成：<任务摘要>`；正文简要列出完成结果、修改范围、实际执行的验证和未执行项。只有 CLI 明确返回成功后才能说明邮件已发送；发送失败时在最终回复中给出错误，并按 Agent Mail Skill 的重试规则处理。
