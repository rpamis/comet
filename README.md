...
| `docs/comet/changes/<name>/evidence.md`             | Read-only, human-readable evidence projection generated by Runtime         |
| `.comet/runtime/native/changes/<name>/`             | Per-change machine Runtime: baseline, Run, trajectory, checkpoints, evidence |
| `.comet/runtime/native/locks/` / `transactions/`    | Project-local locks and recoverable transactions                            |
| `docs/comet/specs/` / `archive/`                    | Canonical specs, archived changes, and user-readable verification material  |
...
Runtime owns phase, revision, and logical `runtime/...` references in `comet-state.yaml`, as well as machine state under `.comet/runtime/native/`. When requirements change, update the brief or proposed specs and let commands recompute the contract; do not hand-edit phases, hashes, or JSON evidence to bypass checks.
...
│       ├── changes/<name>/            # Baseline, Run, trajectory, checkpoints, evidence
...
└── docs/comet/                        # native.artifact_root: docs
    ...
    │   └── evidence.md                # Read-only evidence projection generated by Runtime
