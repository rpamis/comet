...
export function classicCommandHelp(command: string, args: readonly string[]): string | undefined {
  ...
  const usage: Record<string, string[]> = {
    check: [
      ...
      'Default inputs are the working-tree files. Declare git: "all" for builds that read HEAD or the index,',
      'env variable names whose values affect the result, and taskCheckboxes: "include" when completion marks matter.',
      'files accept literal paths and * ? ** globs; a new file matching a glob is checked without re-declaring.',
      'Declare every real input; do not infer dependencies from the current diff.',
      'Policy example: {"version":2,"commands":[{"argv":["node","scripts/build.mjs"],"cwd":".","files":["src/**","package.json"],"git":"all"},{"argv":["node","--test"],"cwd":".","files":["src/**","test/**"],"taskCheckboxes":"ignore"}]}',
